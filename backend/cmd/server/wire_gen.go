...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	subscriptionEntitlementRepository := repository.NewSubscriptionEntitlementRepository(client)
	subscriptionEntitlementPlanRepository := repository.NewSubscriptionEntitlementPlanRepository(client)
	subscriptionEntitlementService := service.NewSubscriptionEntitlementService(subscriptionEntitlementRepository, subscriptionEntitlementPlanRepository)
	concurrencyCache := repository.ProvideConcurrencyCache(redisClient, configConfig)
	schedulerCache := repository.ProvideSchedulerCache(redisClient, configConfig)
	accountRepository := repository.NewAccountRepository(client, db, schedulerCache)
	concurrencyService := service.ProvideConcurrencyService(concurrencyCache, accountRepository, configConfig)
	apiKeyService := service.ProvideAPIKeyService(apiKeyRepository, userRepository, groupRepository, userSubscriptionRepository, userGroupRateRepository, apiKeyCache, configConfig, billingCacheService, settingService, subscriptionEntitlementService, concurrencyService)
	apiKeyAuthCacheInvalidator := service.ProvideAPIKeyAuthCacheInvalidator(apiKeyService)
	promoService := service.NewPromoService(promoCodeRepository, userRepository, billingCacheService, client, apiKeyAuthCacheInvalidator)
	subscriptionService := service.ProvideSubscriptionService(groupRepository, userSubscriptionRepository, billingCacheService, client, configConfig, settingService, subscriptionEntitlementService)
	...
	subscriptionPlanExternalMappingService := service.NewSubscriptionPlanExternalMappingService(subscriptionPlanExternalMappingRepository)
	redeemService := service.ProvideRedeemService(redeemCodeRepository, userRepository, subscriptionService, redeemCache, billingCacheService, client, apiKeyAuthCacheInvalidator, affiliateService, settingService, subscriptionEntitlementService, subscriptionPlanExternalMappingService)
	...
	upstreamRateMultiplierSyncService := service.ProvideUpstreamRateMultiplierSyncService(configConfig, accountRepository, secretEncryptor, rateMultiplierPriorityService)
	opsService := service.ProvideOpsService(opsRepository, settingRepository, configConfig, accountRepository, userRepository, concurrencyService, gatewayService, openAIGatewayService, geminiMessagesCompatService, antigravityGatewayService, opsSystemLogSink, settingService, upstreamRateMultiplierSyncService)
	usageHandler := handler.NewUsageHandler(usageService, apiKeyService, opsService, settingService)
	redeemHandler := handler.NewRedeemHandler(redeemService)
	subscriptionHandler := handler.NewSubscriptionHandler(subscriptionService)
	entitlementHandler := handler.NewEntitlementHandler(subscriptionEntitlementService)
	announcementRepository := repository.NewAnnouncementRepository(client)
	announcementReadRepository := repository.NewAnnouncementReadRepository(client)
	announcementService := service.NewAnnouncementService(announcementRepository, announcementReadRepository, userRepository, userSubscriptionRepository)
	announcementHandler := handler.NewAnnouncementHandler(announcementService)
	supportIssueRepository := repository.NewSupportIssueRepository(client)
	supportIssueService := service.NewSupportIssueService(supportIssueRepository)
	supportIssueHandler := handler.NewSupportIssueHandler(supportIssueService)
	...
	accountHandler := admin.ProvideAccountHandler(adminService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, rateLimitService, accountUsageService, accountTestService, concurrencyService, crsSyncService, sessionLimitCache, rpmCache, compositeTokenCacheInvalidator, grokQuotaService, upstreamRateMultiplierSyncService)
	adminAnnouncementHandler := admin.NewAnnouncementHandler(announcementService)
	adminSupportIssueHandler := admin.NewSupportIssueHandler(supportIssueService)
	...
	upstreamConnectionHandler := admin.NewUpstreamConnectionHandler(upstreamConnectionService)
	...
	referralAdminRelationRepository := repository.ProvideReferralAdminRelationRepository(client)
	referralAdminCommissionRepository := repository.ProvideReferralAdminCommissionRepository(client)
	referralAdminService := service.NewReferralAdminService(referralService, referralAdminRelationRepository, referralAdminCommissionRepository, client, referralSettlementService)
	referralWithdrawalService := service.NewReferralWithdrawalService(commissionRepository, userRepository, client, settingService, referralSettlementService, secretEncryptor)
	referralHandler := admin.NewReferralHandler(referralAdminService, referralWithdrawalService)
	rechargeOrderHandler := admin.NewRechargeOrderHandler(referralRewardService)
	...
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, adminSupportIssueHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, upstreamConnectionHandler, contentModerationHandler, promptAdminHandler, paymentHandler, referralHandler, rechargeOrderHandler, affiliateHandler, complianceHandler, auditLogHandler, upstreamBillingProbeService)
	...
	referralCenterService := service.NewReferralCenterService(referralService, referralCenterRelationRepository, referralCenterCommissionRepository, referralSettlementService)
	handlerReferralHandler := handler.NewReferralHandler(referralService, referralCenterService, referralWithdrawalService)
	lobeHubLaunchStateStore := repository.NewLobeHubLaunchStateStore(redisClient)
	lobeHubUserPreferenceStore := service.ProvideLobeHubUserPreferenceStore(userRepository)
	lobeHubOIDCStateStore := repository.NewLobeHubOIDCStateStore(redisClient)
	lobeHubOIDCSigningKeyProvider := repository.NewLobeHubOIDCSigningKeyProvider()
	lobeHubSSOService := service.ProvideLobeHubSSOService(settingService, lobeHubUserPreferenceStore, apiKeyService, lobeHubOIDCStateStore, lobeHubOIDCSigningKeyProvider)
	lobeHubLaunchService := service.ProvideLobeHubLaunchService(settingService, apiKeyService, lobeHubLaunchStateStore, lobeHubSSOService)
	lobeHubOIDCService := service.ProvideLobeHubOIDCService(settingService, userRepository, lobeHubOIDCStateStore, lobeHubOIDCSigningKeyProvider)
	...
	modelPriceHandler := handler.NewModelPriceHandler(channelService, accountService, apiKeyService, groupService, pricingService, settingService, paymentConfigService)
	...
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, entitlementHandler, announcementHandler, supportIssueHandler, channelMonitorUserHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, handlerPaymentHandler, paymentWebhookHandler, handlerReferralHandler, lobeHubHandler, availableChannelHandler, modelPriceHandler, asyncImageHandler, batchImageHandler, idempotencyCoordinator, idempotencyCleanupService)
	...
	opsAlertEvaluatorService := service.ProvideOpsAlertEvaluatorService(opsService, opsRepository, emailService, accountTestService, redisClient, configConfig, proxyRepository)
