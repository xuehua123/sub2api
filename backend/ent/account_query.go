...
type AccountQuery struct {
	...
	withUpstreamBinding *UpstreamAccountBindingQuery
...
// QueryUpstreamBinding chains the current query on the "upstream_binding" edge.
func (_q *AccountQuery) QueryUpstreamBinding() *UpstreamAccountBindingQuery {
...
func (_q *AccountQuery) Clone() *AccountQuery {
	...
	return &AccountQuery{
		...
		withUpstreamBinding: _q.withUpstreamBinding.Clone(),
...
// WithUpstreamBinding tells the query-builder to eager-load the nodes that are connected to
// the "upstream_binding" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *AccountQuery) WithUpstreamBinding(opts ...func(*UpstreamAccountBindingQuery)) *AccountQuery {
	query := (&UpstreamAccountBindingClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withUpstreamBinding = query
	return _q
}
...
func (_q *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := _q.withUpstreamBinding; query != nil {
		if err := _q.loadUpstreamBinding(ctx, query, nodes, nil,
			func(n *Account, e *UpstreamAccountBinding) { n.Edges.UpstreamBinding = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *AccountQuery) loadUpstreamBinding(ctx context.Context, query *UpstreamAccountBindingQuery, nodes []*Account, init func(*Account), assign func(*Account, *UpstreamAccountBinding)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Account)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(upstreamaccountbinding.FieldAccountID)
	}
	query.Where(predicate.UpstreamAccountBinding(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(account.UpstreamBindingColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
