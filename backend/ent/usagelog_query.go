...
func (_q *UsageLogQuery) QueryEntitlement() *SubscriptionEntitlementQuery {
...
func (_q *UsageLogQuery) WithEntitlement(opts ...func(*SubscriptionEntitlementQuery)) *UsageLogQuery {
	query := (&SubscriptionEntitlementClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UsageLogQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*UsageLog, error) {
	...
	if query := _q.withEntitlement; query != nil {
		if err := _q.loadEntitlement(ctx, query, nodes, nil,
			func(n *UsageLog, e *SubscriptionEntitlement) { n.Edges.Entitlement = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UsageLogQuery) loadEntitlement(ctx context.Context, query *SubscriptionEntitlementQuery, nodes []*UsageLog, init func(*UsageLog), assign func(*UsageLog, *SubscriptionEntitlement)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*UsageLog)
	for i := range nodes {
...
func (_q *UsageLogQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withEntitlement != nil {
			_spec.Node.AddColumnOnce(usagelog.FieldEntitlementID)
		}
	}
