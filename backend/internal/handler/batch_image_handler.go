...
func batchImageOwnerFromContext(c *gin.Context) (service.BatchImageOwner, bool) {
	...
	owner := service.BatchImageOwner{
		...
		EntitlementBalanceFallback: middleware.GetSubscriptionEntitlementBalanceFallbackFromContext(c),
	}
	if subscription, exists := middleware.GetSubscriptionFromContext(c); exists && subscription != nil && subscription.ID > 0 {
		owner.SubscriptionID = &subscription.ID
	}
	if entitlement, exists := middleware.GetSubscriptionEntitlementFromContext(c); exists && entitlement != nil && entitlement.ID > 0 {
		owner.EntitlementID = &entitlement.ID
	}
	return owner, true
}
