...
func stampOpenAIRequestedReasoningEffort(result *service.OpenAIForwardResult, c *gin.Context) {
	...
	// The handler snapshot is captured from the inbound body before group policy,
	// model-family rewriting, or a WSv2 transport adapter can mutate the request.
	// When it exists it is authoritative, even if a downstream forwarder already
	// populated the result from its effective body.
