...
func (s *FailoverState) HandleFailoverError(
	...
	sameAccountRetry := failoverErr.RetryableOnSameAccount &&
		!service.IsChannelMonitorProbe(ctx) &&
	...
	// Channel monitor probes must fail over immediately so one slow account cannot
	// consume the monitor's retry budget before the next account is checked.
	if failoverErr.RetryableOnSameAccount && !service.IsChannelMonitorProbe(ctx) && s.SameAccountRetryCount[accountID] < retryLimit {
