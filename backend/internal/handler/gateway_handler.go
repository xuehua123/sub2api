...
func (h *GatewayHandler) Messages(c *gin.Context) {
	...
	for {
		...
		for {
			...
			submitForwardUsage := func(result *service.ForwardResult) error {
				// 捕获请求信息（用于异步记录，避免在 goroutine 中访问 gin.Context）
				userAgent := c.GetHeader("User-Agent")
				clientIP := ip.GetClientIP(c)
				...
				if !reqStream {
					return runUsageRecordTaskSync(c.Request.Context(), gatewayUsageRecordTask(h.gatewayService, usageInput))
				}
				h.submitUsageRecordTask(c.Request.Context(), func(ctx context.Context) {
					if err := h.gatewayService.RecordUsage(ctx, usageInput); err != nil {
						logger.L().With(
			...
			if err != nil {
				...
				// 不会走到这里重复计费。
				if result != nil {
					if usageErr := submitForwardUsage(result); usageErr != nil {
						reqLog.Error("gateway.partial_usage_record_failed",
							zap.Int64("account_id", account.ID),
							zap.Error(usageErr),
						)
					}
			...
			if err := submitForwardUsage(result); err != nil {
				deferredResponse.Discard()
				status, code, message, retryAfter := billingErrorDetails(err)
				if retryAfter > 0 {
					c.Header("Retry-After", strconv.Itoa(retryAfter))
...
func (h *GatewayHandler) CountTokens(c *gin.Context) {
	...
	// 获取订阅信息（可能为nil）
	subscription, _ := middleware2.GetSubscriptionFromContext(c)
	subscriptionEntitlement, _ := subscriptionEntitlementUsageContext(c)
...
func billingErrorDetails(err error) (status int, code, message string, retryAfter int) {
	...
	if errors.Is(err, service.ErrSubscriptionMaintenance) {
		msg := pkgerrors.Message(err)
		return http.StatusServiceUnavailable, "SUBSCRIPTION_MAINTENANCE_FAILED", msg, 0
	}
