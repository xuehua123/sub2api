...
func (h *OpenAIGatewayHandler) handleGrokMedia(c *gin.Context, endpoint service.GrokMediaEndpoint, requestID string) {
	...
	for {
		...
		if !h.admitEntitlementBeforeForward(c, subscriptionEntitlement, releaseAccount, streamStarted) {
			return
		}
		...
		if endpoint == service.SeedanceEndpointStatus {
			if billResult := prepareSeedanceCompletionBilling(requestCtx, h, apiKey, subject, requestID, result); billResult != nil {
				recordGrokMediaUsage(c, h, reqLog, apiKey, subject, subscription, subscriptionEntitlement, entitlementBalanceFallback, account, billResult, billResult.Model, body, requestID)
			}
			...
			if billResult := prepareGrokVideoCompletionBilling(requestCtx, h, reqLog, apiKey, subject, taskID, result); billResult != nil {
				recordGrokMediaUsage(c, h, reqLog, apiKey, subject, subscription, subscriptionEntitlement, entitlementBalanceFallback, account, billResult, billResult.Model, body, taskID)
			}
		} else if shouldRecordGrokMediaUsage(endpoint, requestModel, result) {
			recordGrokMediaUsage(c, h, reqLog, apiKey, subject, subscription, subscriptionEntitlement, entitlementBalanceFallback, account, result, requestModel, body, requestID)
		}
