...
func (h *OpenAIGatewayHandler) AlphaSearch(c *gin.Context) {
	...
	for {
		...
		if err == nil {
			...
			if result != nil {
				h.recordAlphaSearchUsage(c, apiKey, account, subscription, subscriptionEntitlement, entitlementBalanceFallback, channelMapping, requestedModel, body, result, subject.UserID)
			}
			return
		}
