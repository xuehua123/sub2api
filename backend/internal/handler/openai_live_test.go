...
func TestLiveCallIdentityPreservesEntitlementBillingContext(t *testing.T) {
	gin.SetMode(gin.TestMode)
	context, _ := gin.CreateTestContext(httptest.NewRecorder())
	context.Request = httptest.NewRequest(http.MethodPost, "/v1/live", nil)
	context.Request.Header.Set("User-Agent", "live-test")
	groupID := int64(44)
	apiKey := &service.APIKey{ID: 22, GroupID: &groupID}
	entitlement := &service.SubscriptionEntitlement{ID: 55}

	identity := liveCallIdentity(context, apiKey, 33, nil, entitlement, true)

	require.Equal(t, apiKey.ID, identity.APIKeyID)
	require.Equal(t, groupID, *identity.GroupID)
	require.Nil(t, identity.SubscriptionID)
	require.Equal(t, entitlement.ID, *identity.EntitlementID)
	require.True(t, identity.EntitlementBalanceFallback)
	require.Equal(t, "live-test", identity.UserAgent)
}
