...
func matchOllamaCloudUsageProxyIdentity(ctx context.Context, client *dbent.Client, account *service.Account) (bool, error) {
	if account.ProxyID == nil {
		return true, nil
	}
	current, err := lockOllamaCloudUsageProxyIdentity(ctx, client, *account.ProxyID)
	...
	return current == ollamaCloudUsageProxyIdentityFromService(account.Proxy), nil
}
...
func (r *accountRepository) updateOllamaCloudUsageGroup(
	...
	apply := func(txCtx context.Context, client *dbent.Client) error {
		matchesProxy, err := matchOllamaCloudUsageProxyIdentity(txCtx, client, account)
		if err != nil {
			return err
		}
		if !matchesProxy {
