...
func TestLockAndMergeAccountOllamaCloudUsageExtraHandlesNullableGroupIdentity(t *testing.T) {
	ctx := context.Background()
	tx := testEntTx(t)
	account := mustCreateAccount(t, tx.Client(), &service.Account{
		...
		Extra:       map[string]any{"note": "preserved"},
	})
	...
	require.Equal(t, "preserved", merged["note"])
}
...
// 端到端验证：普通（非 ollama）kimi apikey 账号改凭证进入统一分支后，
// 遗留的 Ollama 托管状态会被清理，其它 extra 键不受影响。
func TestUpdateCredentialsPlainCNAPIKeyAccountCleanupIsSemanticallyEquivalent(t *testing.T) {
	...
	account := mustCreateAccount(t, tx.Client(), &service.Account{
		...
		Extra: map[string]any{
			service.OllamaCloudUsageSessionExtraKey:     "cipher:legacy-session",
			service.OllamaCloudUsageAutoRefreshExtraKey: true,
			service.OllamaCloudUsageSnapshotExtraKey:    map[string]any{"status": "ok"},
			"custom_note":                               "keep-me",
		},
	})
...
// Ollama 组身份变化必须清理会话与快照，避免跨账号复用浏览器会话。
func TestUpdateCredentialsPreservesOrClearsOllamaManagedExtraByIdentity(t *testing.T) {
	ctx := context.Background()
	tx := testEntTx(t)
	repo := newAccountRepositoryWithSQL(tx.Client(), tx, nil)
