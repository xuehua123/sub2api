...
func (r *apiKeyRepository) Update(ctx context.Context, key *service.APIKey, fields service.APIKeyUpdateFields) error {
	...
	now := time.Now()
	builder := clientFromContext(ctx, r.client).APIKey.Update().
	...
	if fields.AccessBinding {
		normalizeAPIKeyAccessSourceForPersistence(key)
		builder.SetAccessSource(key.AccessSource)
		if key.SubscriptionEntitlementID != nil {
	...
	if fields.AutoSwitchGroupEnabled {
		builder.SetAutoSwitchGroupEnabled(key.AutoSwitchGroupEnabled)
	}
