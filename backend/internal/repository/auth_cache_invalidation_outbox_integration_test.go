...
func TestAuthCacheInvalidationTriggers_CoverSecurityMutationsOnly(t *testing.T) {
	...
	require.NoError(t, integrationDB.QueryRowContext(ctx, `
		INSERT INTO subscription_entitlements (
			user_id, primary_group_id, name, source_type, status, starts_at, expires_at
		) VALUES ($1, $2, 'auth-outbox', 'manual', 'active', NOW(), NOW() + INTERVAL '1 day')
		RETURNING id`, user.ID, group.ID).Scan(&entitlementID))
	...
	t.Cleanup(func() {
		...
		require.NoError(t, err)
		_, err = integrationDB.ExecContext(ctx, "DELETE FROM subscription_entitlements WHERE id = $1", entitlementID)
		require.NoError(t, err)
	...
	_, err = integrationDB.ExecContext(ctx, "UPDATE api_keys SET access_source = 'entitlement' WHERE id = $1", key.ID)
	require.NoError(t, err)
	require.Equal(t, 1, count(), "local access-source changes must enqueue")
	clear()
	_, err = integrationDB.ExecContext(ctx, "UPDATE api_keys SET access_source = 'balance' WHERE id = $1", key.ID)
	require.NoError(t, err)
	clear()

	_, err = integrationDB.ExecContext(ctx, "UPDATE api_keys SET subscription_entitlement_id = $2 WHERE id = $1", key.ID, entitlementID)
	require.NoError(t, err)
	require.Equal(t, 1, count(), "local entitlement-binding changes must enqueue")
	clear()
	_, err = integrationDB.ExecContext(ctx, "UPDATE api_keys SET subscription_entitlement_id = NULL WHERE id = $1", key.ID)
	require.NoError(t, err)
	clear()

	_, err = integrationDB.ExecContext(ctx, "UPDATE api_keys SET auto_switch_group_enabled = NOT auto_switch_group_enabled WHERE id = $1", key.ID)
	require.NoError(t, err)
	require.Equal(t, 1, count(), "local automatic-group-switch changes must enqueue")
	clear()
	...
	require.NoError(t, err)
	clear()
	groupCapabilityUpdates := []struct {
		name  string
		query string
	}{
		{name: "balance access", query: "UPDATE groups SET balance_enabled = NOT balance_enabled WHERE id = $1"},
		{name: "subscription access", query: "UPDATE groups SET subscription_enabled = NOT subscription_enabled WHERE id = $1"},
		{name: "plan auto-grant", query: "UPDATE groups SET plan_auto_grant_enabled = NOT plan_auto_grant_enabled WHERE id = $1"},
	}
	for _, update := range groupCapabilityUpdates {
		_, err = integrationDB.ExecContext(ctx, update.query, group.ID)
		require.NoError(t, err)
		require.Equal(t, 1, count(), "%s changes must enqueue bound keys", update.name)
		clear()
	}
