...
func (r *batchImageRepository) CreateBatchImageJob(ctx context.Context, params service.CreateBatchImageJobParams) (*service.BatchImageJob, error) {
	...
	if !service.IsValidUsageBillingSource(params.BillingSource) {
		params.BillingSource = service.ResolveUsageBillingSource(params.BillingType, params.SubscriptionID, params.EntitlementID, false)
	}
...
INSERT INTO batch_image_jobs (
    batch_id, user_id, api_key_id, account_id, group_id, subscription_id, entitlement_id,
    billing_type, billing_source, entitlement_balance_fallback,
    held_daily_window_start, held_weekly_window_start, held_monthly_window_start,
...
RETURNING `+batchImageJobColumns,
		...
		params.BillingType, params.BillingSource, params.EntitlementBalanceFallback,
		params.HeldDailyWindowStart, params.HeldWeeklyWindowStart, params.HeldMonthlyWindowStart,
...
id, batch_id, user_id, api_key_id, account_id, group_id, subscription_id, entitlement_id,
billing_type, billing_source, entitlement_balance_fallback,
held_daily_window_start, held_weekly_window_start, held_monthly_window_start,
provider, model, task_name, parent_batch_id, status,
provider_job_name, provider_input_ref, provider_output_ref, gcs_input_uri, gcs_output_uri,
...
func scanBatchImageJob(row rowScanner) (*service.BatchImageJob, error) {
	...
	var apiKeyID, accountID, groupID, subscriptionID, entitlementID sql.NullInt64
	var heldDailyWindowStart, heldWeeklyWindowStart, heldMonthlyWindowStart sql.NullTime
	...
	err := row.Scan(
		&job.ID, &job.BatchID, &job.UserID, &apiKeyID, &accountID, &groupID, &subscriptionID, &entitlementID,
		&job.BillingType, &job.BillingSource, &job.EntitlementBalanceFallback,
		&heldDailyWindowStart, &heldWeeklyWindowStart, &heldMonthlyWindowStart,
	...
	job.SubscriptionID = batchImageNullInt64Ptr(subscriptionID)
	job.EntitlementID = batchImageNullInt64Ptr(entitlementID)
	job.HeldDailyWindowStart = batchImageNullTimePtr(heldDailyWindowStart)
	job.HeldWeeklyWindowStart = batchImageNullTimePtr(heldWeeklyWindowStart)
	job.HeldMonthlyWindowStart = batchImageNullTimePtr(heldMonthlyWindowStart)
