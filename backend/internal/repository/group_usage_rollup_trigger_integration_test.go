...
func createGroupUsageRollupTriggerTestSchema(t *testing.T, ctx context.Context, partitioned bool) string {
	...
	for _, migrationName := range []string{
		"225_group_usage_daily_rollups.sql",
		"226_group_usage_rollup_timezone.sql",
	} {
...
func beginGroupUsageRollupTriggerTestTx(t *testing.T, ctx context.Context, schema string) *sql.Tx {
	...
	// The migrated state defaults to Asia/Shanghai, and the trigger derives the
	// affected bucket from the database session timezone. Keep the test session
	// aligned with that state so this suite is deterministic around UTC midnight.
