...
func TestPrepareUsageLogInsert_PersistsNativeCompactionV2WithoutChangingRequestType(t *testing.T) {
	...
	require.Equal(t, "smallint", usageLogInsertArgTypes[32])
...
func TestPrepareUsageLogInsert_PersistsEntitlementID(t *testing.T) {
	...
	require.Len(t, prepared.args, len(usageLogInsertArgTypes))
	require.Equal(t, sql.NullInt64{Int64: entitlementID, Valid: true}, prepared.args[11])
}
...
func TestShouldUsePreaggregatedTrendRejectsNativeCompactionV2Filter(t *testing.T) {
	nativeCompactionV2 := true
	require.True(t, shouldUsePreaggregatedTrend("day", 0, 0, 0, 0, 0, "", nil, nil, nil, "", nil, nil))
	require.False(t, shouldUsePreaggregatedTrend("day", 0, 0, 0, 0, 0, "", nil, nil, nil, "", nil, &nativeCompactionV2))
}
