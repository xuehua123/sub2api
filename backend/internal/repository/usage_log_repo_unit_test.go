...
func TestPrepareUsageLogInsert_IncludesFirstSSEEventMs(t *testing.T) {
	...
	require.Equal(t, "integer", usageLogInsertArgTypes[37])
	require.Equal(t, nullInt(&firstSSEEventMs), prepared.args[37])
	require.Equal(t, "integer", usageLogInsertArgTypes[38])
	require.Equal(t, nullInt(&firstClientFlushMs), prepared.args[38])
}
