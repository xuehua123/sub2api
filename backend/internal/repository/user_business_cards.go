...
const userBusinessCardsSQL = `COALESCE((SELECT jsonb_agg(card ORDER BY expiry,id) FROM (
 ...
 'daily_remaining',CASE WHEN (e.daily_limit_usd IS NULL OR e.daily_limit_usd<=0) THEN NULL ELSE GREATEST(0,e.daily_limit_usd-CASE WHEN e.daily_window_start IS NULL OR ((e.daily_window_start AT TIME ZONE $7)::date < ($8 AT TIME ZONE $7)::date AND e.expires_at>e.starts_at+interval '24 hours') THEN 0 ELSE e.daily_usage_usd END) END,
 'weekly_remaining',CASE WHEN (e.weekly_limit_usd IS NULL OR e.weekly_limit_usd<=0) THEN NULL ELSE GREATEST(0,e.weekly_limit_usd-CASE WHEN COALESCE(e.weekly_window_start,e.starts_at)+interval '168 hours' <= $8 THEN 0 ELSE e.weekly_usage_usd END) END,
 'monthly_remaining',CASE WHEN (e.monthly_limit_usd IS NULL OR e.monthly_limit_usd<=0) THEN NULL ELSE GREATEST(0,e.monthly_limit_usd-CASE WHEN COALESCE(e.monthly_window_start,e.starts_at)+interval '720 hours' <= $8 THEN 0 ELSE e.monthly_usage_usd END) END,
 ...
 FROM subscription_entitlements e WHERE e.user_id=$9 AND e.deleted_at IS NULL AND e.status='active' AND e.starts_at <= $8 AND e.expires_at>$8
 UNION ALL
 ...
 'daily_remaining',CASE WHEN (g.daily_limit_usd IS NULL OR g.daily_limit_usd<=0) THEN NULL ELSE GREATEST(0,g.daily_limit_usd-CASE WHEN s.daily_window_start IS NULL OR ((s.daily_window_start AT TIME ZONE $7)::date < ($8 AT TIME ZONE $7)::date AND s.expires_at>s.starts_at+interval '24 hours') THEN 0 ELSE s.daily_usage_usd END) END,
 'weekly_remaining',CASE WHEN (g.weekly_limit_usd IS NULL OR g.weekly_limit_usd<=0) THEN NULL ELSE GREATEST(0,g.weekly_limit_usd-CASE WHEN COALESCE(s.weekly_window_start,s.starts_at)+interval '168 hours'<=$8 THEN 0 ELSE s.weekly_usage_usd END) END,
 'monthly_remaining',CASE WHEN (g.monthly_limit_usd IS NULL OR g.monthly_limit_usd<=0) THEN NULL ELSE GREATEST(0,g.monthly_limit_usd-CASE WHEN COALESCE(s.monthly_window_start,s.starts_at)+interval '720 hours'<=$8 THEN 0 ELSE s.monthly_usage_usd END) END)
 FROM user_subscriptions s JOIN groups g ON g.id=s.group_id WHERE s.user_id=$9 AND s.deleted_at IS NULL AND s.status='active' AND s.starts_at<=$8 AND s.expires_at>$8
