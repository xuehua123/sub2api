...
func (s *UserSubscriptionRepoSuite) TestGetByIDIncludeDeleted_PreservesPersistedStatus() {
	...
	ent, err := s.client.SubscriptionEntitlement.Create().
		SetUserID(user.ID).
		SetLegacySubscriptionID(sub.ID).
		SetPrimaryGroupID(group.ID).
		SetName("include deleted linked entitlement").
		SetSourceType(service.SubscriptionEntitlementSourceAdminAssign).
		SetStatus(service.SubscriptionStatusRevoked).
	...
	s.Require().NotNil(got.EntitlementLink)
	s.Require().Equal(ent.ID, got.EntitlementLink.EntitlementID)
	s.Require().Equal(service.SubscriptionStatusRevoked, got.EntitlementLink.Status)
}
