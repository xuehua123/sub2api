...
func TestAPIKeyAuthSubscriptionBillingServiceUnavailableReturns503(t *testing.T) {
	...
	cfg := &config.Config{RunMode: config.RunModeStandard}
	billingCacheService := service.NewBillingCacheService(nil, nil, billingSubRepo, nil, nil, nil, cfg, nil)
...
func TestAPIKeyAuthOpenAIQuotaErrorFormat(t *testing.T) {
	...
	require.Equal(t, service.QuotaInsufficientMessage, response.Error.Message)
...
func TestAPIKeyAuthQuotaErrorKeepsLegacyFormatOutsideResponses(t *testing.T) {
	...
	require.Equal(t, http.StatusTooManyRequests, w.Code)
	requireAPIKeyAuthError(t, w, "API_KEY_QUOTA_EXHAUSTED", service.QuotaInsufficientMessage)
}
...
func (r *stubUserSubscriptionRepo) List(ctx context.Context, params pagination.PaginationParams, userID, groupID *int64, status, platform, sortBy, sortOrder string, extra ...service.SubscriptionAdminFilters) ([]service.UserSubscription, *pagination.PaginationResult, error) {
	return nil, nil, errors.New("not implemented")
}
