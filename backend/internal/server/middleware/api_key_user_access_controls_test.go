...
func TestValidateAPIKeyGroupAllowedEnforcesPublicAllowlistForEveryAccessSource(t *testing.T) {
	t.Parallel()
	...
	key := &service.APIKey{
		...
		Group: &service.Group{ID: groupID, Status: service.StatusActive, BalanceEnabled: true},
	}

	require.False(t, validateAPIKeyGroupAllowed(key, false, service.APIKeyAccessSourceBalance))
	require.False(t, validateAPIKeyGroupAllowed(key, true, service.APIKeyAccessSourceEntitlement))

	key.User.AllowedGroups = append(key.User.AllowedGroups, groupID)
	require.True(t, validateAPIKeyGroupAllowed(key, false, service.APIKeyAccessSourceBalance))
	require.True(t, validateAPIKeyGroupAllowed(key, true, service.APIKeyAccessSourceEntitlement))
}
