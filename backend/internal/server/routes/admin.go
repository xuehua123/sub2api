...
func RegisterAdminRoutes(
	...
	admin.Use(middleware.AdminComplianceGuard(settingService))
	{
		...
		if h.ModelPrice != nil {
			...
			admin.PATCH("/model-prices/hidden-model", h.ModelPrice.UpdateHiddenModel)
			admin.PATCH("/model-prices/custom-price", h.ModelPrice.UpdateCustomPrice)
...
func registerOpsRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	ops := admin.Group("/ops")
	{
		...
		ops.GET("/upstream-connection-balance", h.Admin.Ops.GetUpstreamConnectionBalanceMonitor)
		ops.PATCH("/upstream-connection-balance/settings", h.Admin.Ops.UpdateUpstreamConnectionBalanceSettings)
		ops.POST("/upstream-connection-balance/test-enterprise-wechat", h.Admin.Ops.TestUpstreamConnectionBalanceEnterpriseWeChat)
		ops.PATCH("/upstream-connection-balance/:id", h.Admin.Ops.UpdateUpstreamConnectionBalanceAlert)
		ops.POST("/upstream-connection-balance/:id/probe", h.Admin.Ops.ProbeUpstreamConnectionBalance)
...
func registerAccountRoutes(admin *gin.RouterGroup, h *handler.Handlers, stepUpAuth middleware.StepUpAuthMiddleware) {
	accounts := admin.Group("/accounts")
	{
		...
		accounts.PATCH("/:id/tags", h.Admin.Account.UpdateTags)
...
func registerSettingsRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	adminSettings := admin.Group("/settings")
	{
		...
		adminSettings.GET("/rate-multiplier-priority", h.Admin.Setting.GetRateMultiplierPrioritySettings)
		adminSettings.PUT("/rate-multiplier-priority", h.Admin.Setting.UpdateRateMultiplierPrioritySettings)
		// 面板 API 限流配置
...
func registerSubscriptionRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	subscriptions := admin.Group("/subscriptions")
	{
		...
		subscriptions.POST("/:id/monthly-cycle-adjustments/preview", h.Admin.Subscription.PreviewMonthlyCycleAdjustment)
		subscriptions.POST("/:id/monthly-cycle-adjustments", h.Admin.Subscription.ApplyMonthlyCycleAdjustment)
