...
func RegisterGatewayRoutes(
	...
	gateway.Use(requireGroupAnthropic)
	{
		...
		gateway.POST("/responses/*subpath", channelMonitorProbe, guardResponsesSubpath(func(c *gin.Context) {
			if isOpenAIResponsesCompatibleGatewayPlatform(c) {
				h.OpenAIGateway.Responses(c)
				return
			}
		...
		gateway.POST("/alpha/search", textBodyLimit, channelMonitorProbe, h.OpenAIGateway.AlphaSearch)
	...
	r.POST("/responses", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, channelMonitorProbe, gin.HandlerFunc(apiKeyAuth), compositeTarget, requireGroupAnthropic, responsesHandler)
	r.POST("/responses/*subpath", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, channelMonitorProbe, gin.HandlerFunc(apiKeyAuth), compositeTarget, requireGroupAnthropic, guardResponsesSubpath(responsesHandler))
	r.POST("/alpha/search", textBodyLimit, clientRequestID, opsErrorLogger, endpointNorm, channelMonitorProbe, gin.HandlerFunc(apiKeyAuth), compositeTarget, requireGroupAnthropic, h.OpenAIGateway.AlphaSearch)
	...
	codexDirect.Use(bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), compositeTarget, requireGroupAnthropic)
	{
		...
		codexDirect.POST("/responses/*subpath", channelMonitorProbe, guardResponsesSubpath(responsesHandler))
		codexDirect.POST("/alpha/search", textBodyLimit, channelMonitorProbe, h.OpenAIGateway.AlphaSearch)
	...
	r.POST("/chat/completions", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, channelMonitorProbe, gin.HandlerFunc(apiKeyAuth), compositeTarget, requireGroupAnthropic, func(c *gin.Context) {
		if isOpenAIResponsesCompatibleGatewayPlatform(c) {
			h.OpenAIGateway.ChatCompletions(c)
			return
		}
