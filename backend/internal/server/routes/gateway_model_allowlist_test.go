...
func TestGatewayRoutesGroupModelAllowlistMountedOnEveryGatewayRoute(t *testing.T) {
	...
	rootHelper := regexp.MustCompile(regexp.QuoteMeta(`r.Handle(method, path, limit, clientRequestID, opsErrorLogger, endpointNorm, channelMonitorProbe, gin.HandlerFunc(apiKeyAuth), groupModelAllowlist, requireGroupAnthropic, handler)`))
	...
	codexDirect := regexp.MustCompile(regexp.QuoteMeta(`codexDirect.Use(bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, channelMonitorProbe, gin.HandlerFunc(apiKeyAuth), groupModelAllowlist, requireGroupAnthropic)`))
