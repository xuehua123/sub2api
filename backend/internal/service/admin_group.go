...
func (s *adminServiceImpl) UpdateGroup(ctx context.Context, id int64, input *UpdateGroupInput) (*Group, error) {
	...
	if input.ModelPricing != nil {
		...
		// Entry platforms are persisted API metadata even though runtime matching
		// is group-scoped. Keep that invariant when only the group's platform is
		// patched, otherwise subsequent reads expose stale per-entry platforms.
...
func normalizeGroupModelPricing(platform string, pricing []ChannelModelPricing) ([]ChannelModelPricing, error) {
	...
	for i := range pricing {
		...
		// would let duplicate/overlapping model patterns evade validation and then
		// silently become first-match-wins at runtime.
...
func (s *adminServiceImpl) AdminUpdateAPIKeyGroupID(ctx context.Context, keyID int64, groupID *int64) (*AdminUpdateAPIKeyGroupIDResult, error) {
	...
	if *groupID == 0 {
		...
		if apiKey.User != nil && !apiKey.User.AllowsGroupByPolicy(group.ID, group.IsExclusive) {
		...
		if autoGrantBalanceAccess {
			...
			if err := s.apiKeyRepo.Update(opCtx, apiKey, APIKeyUpdateFields{GroupID: true, AccessBinding: true}); err != nil {
	...
	if err := s.apiKeyRepo.Update(ctx, apiKey, APIKeyUpdateFields{GroupID: true, AccessBinding: true}); err != nil {
		return nil, fmt.Errorf("update api key: %w", err)
	}
