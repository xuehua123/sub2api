...
func cloneGroupModelPricing(value []ChannelModelPricing) []ChannelModelPricing {
	...
	for i := range value {
		cloned[i] = value[i]
		if value[i].Models != nil {
			cloned[i].Models = append([]string{}, value[i].Models...)
		}
		...
		if value[i].Intervals == nil {
			continue
		}
		cloned[i].Intervals = make([]PricingInterval, len(value[i].Intervals))
		for j := range value[i].Intervals {
			cloned[i].Intervals[j] = value[i].Intervals[j]
...
func cloneGroupForDuplicate(source *Group, operationID string) *Group {
	return &Group{
		...
		BalanceEnabled:                  source.BalanceEnabled,
