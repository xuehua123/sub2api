...
func TestUpdateGroupPlatformNormalizesExistingModelPricingWhenPricingOmitted(t *testing.T) {
	inputPrice := 1.25
	...
	repo := &groupPlatformRepoStub{group: &Group{
		ID:           7,
		Name:         "g",
		Platform:     PlatformAnthropic,
		ModelPricing: []ChannelModelPricing{original},
	}}
	svc := &adminServiceImpl{groupRepo: repo}
	...
	require.Equal(t, PlatformOpenAI, got.ModelPricing[0].Platform)
	require.Zero(t, got.ModelPricing[0].ID)
	require.Zero(t, got.ModelPricing[0].ChannelID)
	require.Equal(t, []string{"claude-*"}, got.ModelPricing[0].Models)
	// Normalization must not mutate the repository-owned source object.
	require.Equal(t, PlatformAnthropic, repo.group.ModelPricing[0].Platform)
	require.Equal(t, []string{" claude-* "}, repo.group.ModelPricing[0].Models)
}
