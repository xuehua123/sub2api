...
func TestAPIKeyService_RejectsV21AuthSnapshotWithoutGroupModelPricing(t *testing.T) {
	...
	if ok {
		t.Fatal("expected v21 auth snapshot to be rejected after group pricing fields were added")
	}
	if apiKey != nil {
		t.Fatalf("expected no API key from stale snapshot, got %#v", apiKey)
	}
...
func TestAPIKeyService_RejectsV22AuthSnapshotWithoutUserGroupAccessPolicy(t *testing.T) {
	svc := &APIKeyService{}

	apiKey, ok, err := svc.applyAuthCacheEntry("k-legacy-user-group-policy", &APIKeyAuthCacheEntry{
	...
	if ok {
		t.Fatal("expected v22 auth snapshot to be rejected after user group access policy was added")
	}
	if apiKey != nil {
		t.Fatalf("expected no API key from stale snapshot, got %#v", apiKey)
	}
...
func TestAPIKeyService_RejectsV23AuthSnapshotWithoutPublicGroupAccessPolicy(t *testing.T) {
	svc := &APIKeyService{}

	apiKey, ok, err := svc.applyAuthCacheEntry("k-legacy-public-group-policy", &APIKeyAuthCacheEntry{
	...
	if ok {
		t.Fatal("expected v23 auth snapshot to be rejected after public group access policy was added")
	}
	if apiKey != nil {
		t.Fatalf("expected no API key from stale snapshot, got %#v", apiKey)
	}
