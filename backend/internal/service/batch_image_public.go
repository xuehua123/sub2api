...
func NewBatchImagePublicService(repo BatchImageRepository, accountRepo AccountRepository, groupRepo GroupRepository, userGroupRateRepo UserGroupRateRepository, queue BatchImageQueue, pricing *BatchImageModelPricingResolver, billingRepo UsageBillingRepository, authCache APIKeyAuthCacheInvalidator, billingCache *BillingCacheService, cfg *config.Config) *BatchImagePublicService {
	return &BatchImagePublicService{
		...
		AuthCache:         authCache,
		SubscriptionCache: billingCache,
		Config:            cfg,
	}
}

func batchImageBillingIdentity(owner BatchImageOwner) (int8, string) {
	if owner.EntitlementID != nil {
		return BillingTypeSubscription, BillingSourceEntitlementQuota
	}
	if owner.SubscriptionID != nil {
		return BillingTypeSubscription, BillingSourceLegacySubscription
	}
	return BillingTypeBalance, BillingSourceBalance
}
...
func (s *BatchImagePublicService) Submit(ctx context.Context, owner BatchImageOwner, req BatchImageSubmitRequest, idempotencyKey string) (*BatchImagePublicBatch, error) {
	...
	billingType, billingSource := batchImageBillingIdentity(owner)
	job, err := s.Repo.CreateBatchImageJob(ctx, CreateBatchImageJobParams{
		...
		EntitlementBalanceFallback: owner.EntitlementBalanceFallback,
...
func (s *BatchImagePublicService) releaseFailedSubmitHold(ctx context.Context, job *BatchImageJob, requestHash string) error {
	if err := releaseBatchImageBalanceHold(ctx, s.BillingRepo, s.SubscriptionCache, job, requestHash); err != nil {
