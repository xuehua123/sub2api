...
func TestBatchImagePublicService_Submit(t *testing.T) {
	...
	t.Run("persists resolved subscription billing identity into the job and hold", func(t *testing.T) {
		...
		require.Equal(t, &subscriptionID, job.SubscriptionID)
		require.Equal(t, &entitlementID, job.EntitlementID)
		require.Equal(t, BillingTypeSubscription, job.BillingType)
		require.Equal(t, BillingSourceEntitlementQuota, job.BillingSource)
		require.True(t, job.EntitlementBalanceFallback)

		billing := svc.BillingRepo.(*fakeBatchImageBillingRepo)
		require.Len(t, billing.reserves, 1)
		hold := billing.reserves[0]
		require.Equal(t, &groupID, hold.GroupID)
		require.Equal(t, &subscriptionID, hold.SubscriptionID)
		require.Equal(t, &entitlementID, hold.EntitlementID)
		require.Equal(t, BillingTypeSubscription, hold.BillingType)
		require.Equal(t, BillingSourceEntitlementQuota, hold.BillingSource)
		require.True(t, hold.EntitlementBalanceFallback)
	})
