...
type ModelPricing struct {
	...
	LongContextCacheReadMultiplier     float64  // 长上下文整次会话缓存读取倍率
	ImageOutputPricePerToken           float64  // 图片输出 token 价格 (USD)
	ImageOutputPriceExplicit           bool     // 是否由渠道定价显式设定（为 true 时即使 == 0 也不回退）
}

const (
	openAIGPT54PriorityInputPrice     = 5e-6
	openAIGPT54PriorityOutputPrice    = 30e-6
	openAIGPT54PriorityCacheReadPrice = 0.5e-6
...
func isAnthropicFastBillingModel(model string) bool {
...
func (s *BillingService) computeTokenBreakdown(
	...
	isPriorityTier := normalizedServiceTier == "priority" || normalizedServiceTier == "fast"
	applySessionLongContext := applyLongCtx && s.shouldApplySessionLongContextPricing(tokens, pricing)
	useStandardRatesForPriorityLongContext := isPriorityTier &&
		applySessionLongContext && isOpenAIGPT54Model(model) && !isOpenAIGPT55Model(model)

	if isPriorityTier && pricing.FastMultiplier != nil {
		// An operator-configured multiplier is the strongest pricing signal. It
		// intentionally overrides both catalog priority prices and built-in model
		// ratios, including for providers that add Fast to otherwise unknown models.
		tierMultiplier = *pricing.FastMultiplier
	} else if useStandardRatesForPriorityLongContext {
		// OpenAI processes GPT-5.4 priority prompts above 272K at standard long-context rates.
	...
	} else if isPriorityTier {
		// Only models with an explicit built-in Fast policy receive an implicit
		// surcharge. Known-unsupported and unknown models stay at the standard
		// rate unless an operator/provider supplied a multiplier or tier prices.
		normalizedModel := normalizeKnownOpenAICodexModel(model)
		if ratio := openAIModelFastPricingRatio(normalizedModel); ratio > 0 {
			tierMultiplier = ratio
		} else if normalizedServiceTier == "fast" && isAnthropicFastBillingModel(model) {
			// Anthropic usage records use speed=fast directly (OpenAI's public
			// alias is normalized to priority before billing). Preserve that
			// protocol's established 2x fallback without reintroducing a surcharge
			// for unknown models that happen to carry a literal fast tier.
			tierMultiplier = serviceTierCostMultiplier(normalizedServiceTier)
		}
	} else {
	...
	if applySessionLongContext {
		...
		if pricing.LongContextCacheReadMultiplier > 0 {
			cacheReadPrice *= pricing.LongContextCacheReadMultiplier
		} else {
			// Cache reads are reused input tokens; apply the input long-context multiplier
...
func (s *BillingService) calculatePerRequestCost(resolved *ResolvedPricing, input CostInput) (*CostBreakdown, error) {
	...
	if input.SizeTier != "" {
		unitPrice, found = input.Resolver.GetRequestTierPrice(resolved, input.SizeTier)
	}

	if !found {
		totalContext := input.Tokens.InputTokens + input.Tokens.CacheCreationTokens + input.Tokens.CacheReadTokens
		unitPrice, found = input.Resolver.GetRequestTierPriceByContext(resolved, totalContext)
	}
...
func (s *BillingService) applyModelSpecificPricingPolicyEx(model string, pricing *ModelPricing, forceDeepSeekRates bool) *ModelPricing {
	...
	needsPriorityPolicy := normalized == "gpt-5.4" &&
	...
	if !needsPriorityPolicy && !needsCacheCreationPolicy && fastRatio <= 0 && !needsMaxReasoningEffortMultiplier {
		return pricing
	}
	cloned := *pricing
	...
	if needsPriorityPolicy {
		cloned.InputPricePerTokenPriority = math.Max(cloned.InputPricePerTokenPriority, openAIGPT54PriorityInputPrice)
		cloned.OutputPricePerTokenPriority = math.Max(cloned.OutputPricePerTokenPriority, openAIGPT54PriorityOutputPrice)
		cloned.CacheReadPricePerTokenPriority = math.Max(cloned.CacheReadPricePerTokenPriority, openAIGPT54PriorityCacheReadPrice)
	}
...
func enforceOpenAIFastPricingRatio(pricing *ModelPricing, ratio float64) {
	...
	pricing.InputPricePerTokenPriority = math.Max(pricing.InputPricePerTokenPriority, pricing.InputPricePerToken*ratio)
	pricing.OutputPricePerTokenPriority = math.Max(pricing.OutputPricePerTokenPriority, pricing.OutputPricePerToken*ratio)
	if pricing.CacheReadPricePerToken > 0 {
		pricing.CacheReadPricePerTokenPriority = math.Max(pricing.CacheReadPricePerTokenPriority, pricing.CacheReadPricePerToken*ratio)
	}
	if pricing.CacheCreationPricePerToken > 0 {
		pricing.CacheCreationPricePerTokenPriority = math.Max(pricing.CacheCreationPricePerTokenPriority, pricing.CacheCreationPricePerToken*ratio)
	}
}
