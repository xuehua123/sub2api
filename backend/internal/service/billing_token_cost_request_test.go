...
func TestGatewayCalculateTokenCost_GroupNilDeepSeekUsesResolverPeakSchedule(t *testing.T) {
	...
	gateway := &GatewayService{billingService: bs, resolver: resolver}
	result := &ForwardResult{Usage: ClaudeUsage{
		InputTokens: 1_000_000, OutputTokens: 1_000_000, CacheReadInputTokens: 1_000_000,
	}}
	apiKey := &APIKey{Group: nil}
	...
	for _, tc := range []struct {
		...
		{name: "weekday off-peak", pricingAt: time.Date(2026, 8, 24, 5, 0, 0, 0, time.UTC), multiplier: 1},
		{name: "weekday peak", pricingAt: time.Date(2026, 8, 24, 2, 0, 0, 0, time.UTC), multiplier: 2},
	} {
		t.Run(tc.name, func(t *testing.T) {
			got := gateway.calculateTokenCost(context.Background(), result, apiKey, model, 1, tc.pricingAt)
			require.InDelta(t, deepseekProOffPeakInputPrice*1_000_000*tc.multiplier, got.InputCost, 1e-12)
			require.InDelta(t, deepseekProOffPeakOutputPrice*1_000_000*tc.multiplier, got.OutputCost, 1e-12)
			require.InDelta(t, deepseekProOffPeakCacheRead*1_000_000*tc.multiplier, got.CacheReadCost, 1e-12)
		})
	}
}

func TestGatewayCalculateTokenCost_GroupNilWithoutResolverKeepsLegacyFallback(t *testing.T) {
	const model = "deepseek-v4-pro"
	bs := NewBillingService(&config.Config{}, nil)
	gateway := &GatewayService{billingService: bs}
	result := &ForwardResult{Usage: ClaudeUsage{InputTokens: 1_000_000, OutputTokens: 1_000_000}}
	peak := time.Date(2026, 8, 24, 2, 0, 0, 0, time.UTC)

	got := gateway.calculateTokenCost(context.Background(), result, &APIKey{Group: nil}, model, 1, peak)
