...
func TestGPT56IntervalInputOverrideRecomputesImplicitCacheWritePrice(t *testing.T) {
	base := &ModelPricing{
	...
	resolved := &ResolvedPricing{
		...
		Intervals: []PricingInterval{{
			MinTokens:  0,
			InputPrice: pricingMultiplier(4e-6),
		}},
	}

	pricing := (&ModelPricingResolver{}).GetIntervalPricing(resolved, 1)
	pricing = (&BillingService{}).applyModelSpecificPricingPolicy("gpt-5.6-sol", pricing)
