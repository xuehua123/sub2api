...
func TestHandleCCBufferedFromAnthropic_PreservesMessageStartCacheUsageAndReasoning(t *testing.T) {
	...
	resp := &http.Response{
		Header: http.Header{"Content-Type": []string{"text/event-stream"}, "Content-Encoding": []string{"gzip"}, "x-request-id": []string{"rid_cc_buffered"}},
	...
	require.Equal(t, "rid_cc_buffered", rec.Header().Get("x-request-id"))
}
