...
func TestSkippedErrorPolicyFailoverError(t *testing.T) {
	...
	tests := []struct {
		...
		{"pool_500_failover_no_same_account_retry", poolAccount(nil), 500, false, true, false},
		{"pool_429_failover_with_same_account_retry", poolAccount(nil), 429, false, true, true},
		{"pool_custom_retry_codes_500", poolAccount(map[string]any{
			"pool_mode_retry_status_codes": []any{float64(500)},
		}), 500, false, true, true},
		{"pool_400_not_failover_worthy", poolAccount(nil), 400, false, false, false},
		{"monitor_pool_400_fails_over_to_next_account", poolAccount(nil), 400, true, true, false},
		{"custom_codes_miss_500_failover_no_same_account_retry", customCodesAccount, 500, false, true, false},
		{"custom_codes_miss_400_not_failover_worthy", customCodesAccount, 400, false, false, false},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			c.Request = httptest.NewRequest(http.MethodPost, "/v1/messages", nil)
			if tt.monitorProbe {
				c.Request = c.Request.WithContext(WithChannelMonitorProbe(c.Request.Context()))
			}
