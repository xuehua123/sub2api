...
func (s *GeminiMessagesCompatService) poolModeSkippedFailoverError(c *gin.Context, account *Account, statusCode int, respBody []byte, upstreamRequestID string) *UpstreamFailoverError {
	shouldFailover := s.shouldFailoverGeminiUpstreamError(statusCode)
	if c != nil && c.Request != nil {
		shouldFailover = s.shouldFailoverGeminiUpstreamErrorForContext(c.Request.Context(), statusCode)
	}
	if !account.IsPoolMode() || !shouldFailover {
		return nil
	}
	upstreamMsg := sanitizeUpstreamErrorMessage(strings.TrimSpace(extractUpstreamErrorMessage(respBody)))
	upstreamDetail := ""
