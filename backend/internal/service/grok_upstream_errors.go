...
// shouldFailoverGrokUpstreamErrorForContext preserves the monitor probe
// override used by the generic OpenAI path while keeping request-scoped Grok
// content-policy refusals on the current account.
func (s *OpenAIGatewayService) shouldFailoverGrokUpstreamErrorForContext(ctx context.Context, statusCode int, responseBody []byte) bool {
	if isGrokContentPolicyRejection(statusCode, responseBody) {
		return false
	}
	if s.shouldFailoverGrokUpstreamError(statusCode, responseBody) {
		return true
	}
...
func (s *OpenAIGatewayService) shouldFailoverGrokUpstreamError(statusCode int, responseBody []byte) bool {
	...
	case GrokFailureFreeUsage, GrokFailureEmptyUpstream, GrokFailureBilling, GrokFailureModelCapacity, GrokFailureModelUnavailable, GrokFailureCompatibility:
