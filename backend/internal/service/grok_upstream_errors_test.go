...
func TestGrokContentPolicy403DoesNotMutateOrFailover(t *testing.T) {
	...
	require.False(t, svc.shouldFailoverGrokUpstreamErrorForContext(context.Background(), http.StatusForbidden, body))
	require.False(t, svc.shouldFailoverGrokUpstreamErrorForContext(WithChannelMonitorProbe(context.Background()), http.StatusForbidden, body))
	require.True(t, svc.shouldFailoverGrokUpstreamErrorForContext(WithChannelMonitorProbe(context.Background()), http.StatusBadRequest, []byte(`{"error":{"message":"bad monitor request"}}`)))
...
func TestHandleGrokAccountUpstreamErrorDefaultCooldownsRespectPoolMode(t *testing.T) {
	for _, statusCode := range []int{
		...
		t.Run(http.StatusText(statusCode), func(t *testing.T) {
			...
			require.True(t, svc.shouldFailoverGrokUpstreamErrorForContext(context.Background(), statusCode, body))
		})
	}
