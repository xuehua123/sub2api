...
func (c *IdempotencyCoordinator) Execute(
	...
	responseLimit := c.cfg.MaxStoredResponseLen
	if opts.PreserveFullResponse {
		responseLimit = 0
	}
	storedBody, marshalErr := marshalStoredResponseWithLimit(data, responseLimit)
	if marshalErr != nil {
...
func (c *IdempotencyCoordinator) marshalStoredResponse(data any) (string, error) {
	return marshalStoredResponseWithLimit(data, c.cfg.MaxStoredResponseLen)
}

func marshalStoredResponseWithLimit(data any, limit int) (string, error) {
	...
	if limit > 0 && len(redacted) > limit {
		redacted = truncateUTF8(redacted, limit) + "...(truncated)"
	}
	return redacted, nil
}
