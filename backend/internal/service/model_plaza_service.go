...
func (s *ModelPlazaService) ListGroups(ctx context.Context) ([]PlazaGroup, error) {
	...
	displayPricingService := &ChannelService{
	...
	for i := range channels {
		...
		for _, gid := range ch.GroupIDs {
			...
			for j := range supported {
				...
				pricing, groupOverride := displayPricingService.PricingForGroupDisplay(groupEnt[gid], billingModel, rawPricing[j])
				if pricing == nil {
				...
				if at, seen := idx[key]; seen {
					...
					if pg.Models[at].Pricing == nil && pricing != nil {
						...
						pg.Models[at].groupPricingOverride = groupOverride
					}
					continue
				}
				...
				pg.Models = append(pg.Models, PlazaModel{
					...
					groupPricingOverride: groupOverride,
				})
			}
		}
	}
...
func (s *ModelPlazaService) lookupOfficialPricing(ctx context.Context, modelName string, memo map[string]*PlazaOfficialPricing) *PlazaOfficialPricing {
	...
	} else if mp, err := s.billingService.GetModelPricing(modelName); err == nil && mp != nil {
		result = &PlazaOfficialPricing{
	...
	if result != nil && result.InputPrice == nil && result.ImageInputPrice == nil && result.OutputPrice == nil && result.CacheWritePrice == nil &&
