...
type ResolvedPricing struct {
	// model 保留解析时的模型名，供区间覆盖后重新应用模型专属策略。
	model string
...
func (r *ModelPricingResolver) resolveConfiguredPricing(config *ChannelModelPricing, model, source string) *ResolvedPricing {
	...
	resolved := &ResolvedPricing{model: model, Mode: mode, Source: source, channelPricing: config}
	if mode == BillingModePerRequest || mode == BillingModeImage || mode == BillingModeVideo {
...
func (r *ModelPricingResolver) GetIntervalPricing(resolved *ResolvedPricing, totalContextTokens int) *ModelPricing {
	...
	pricing := intervalToModelPricingForModel(resolved.model, iv, resolved.BasePricing, resolved.channelPricing)
...
// intervalToModelPricingForModel keeps model-derived prices consistent when an
// interval overrides the input price but intentionally leaves cache-write
// pricing implicit. GPT-5.6 defines cache creation as 1.25x input; carrying the
// catalog's old absolute cache price across the interval would undercharge and
// make the admin display disagree with runtime billing.
func intervalToModelPricingForModel(model string, iv *PricingInterval, base *ModelPricing, chPricing *ChannelModelPricing) *ModelPricing {
	pricing := intervalToModelPricing(iv, base, chPricing)
	if iv == nil || !isOpenAIGPT56Model(normalizeKnownOpenAICodexModel(model)) ||
		iv.InputPrice == nil || iv.CacheWritePrice != nil || iv.CacheWriteMultiplier != nil ||
		(chPricing != nil && chPricing.CacheWritePrice != nil) {
		return pricing
	}
	pricing.CacheCreationPricePerToken = 0
	pricing.CacheCreationPricePerTokenPriority = 0
	pricing.CacheCreationPriceExplicit = false
...
// found 用于区分“显式配置为 0”和“未找到配置”，避免免费档位错误回退到其他价格。
func (r *ModelPricingResolver) GetRequestTierPrice(resolved *ResolvedPricing, tierLabel string) (price float64, found bool) {
...
// found 用于区分“显式配置为 0”和“未找到配置”。
func (r *ModelPricingResolver) GetRequestTierPriceByContext(resolved *ResolvedPricing, totalContextTokens int) (price float64, found bool) {
