...
func (r *ollamaUsageTestRepo) GetByID(ctx context.Context, id int64) (*Account, error) {
	...
	return r.ollamaUsageTestAccountStore.GetByID(ctx, id)
}

type ollamaUsageTestAccountStore struct {
	AccountRepository
	mu       sync.Mutex
	accounts map[int64]*Account
}

func newOllamaUsageTestRepo(accounts map[int64]*Account) *ollamaUsageTestRepo {
	return &ollamaUsageTestRepo{
		ollamaUsageTestAccountStore: &ollamaUsageTestAccountStore{accounts: accounts},
	}
}

func (r *ollamaUsageTestAccountStore) GetByID(_ context.Context, id int64) (*Account, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	clone := cloneOllamaUsageTestAccount(*account)
	return &clone, nil
}

type ollamaUsageTestSettingRepo struct {
	SettingRepository
	mu     sync.Mutex
	values map[string]string
}

func (r *ollamaUsageTestSettingRepo) GetValue(_ context.Context, key string) (string, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *ollamaUsageTestSettingRepo) Set(_ context.Context, key, value string) error {
	r.mu.Lock()
	defer r.mu.Unlock()
	if r.values == nil {
...
func TestOllamaCloudUsageSettingsDefaultOffAndValidation(t *testing.T) {
	repo := &ollamaUsageTestSettingRepo{}
...
func TestOllamaCloudUsageSessionEncryptionFailClosedAndWriteOnlyState(t *testing.T) {
	...
	repo := newOllamaUsageTestRepo(map[int64]*Account{7: account})
	settings := &ollamaUsageTestSettingRepo{}
...
func TestOllamaCloudUsageGroupSharesAcrossPlatformsURLVariantsAndDynamicSiblings(t *testing.T) {
	...
	repo := newOllamaUsageTestRepo(map[int64]*Account{
		source.ID: source, sibling.ID: sibling, different.ID: different,
	})
	svc := newOllamaUsageTestService(t, repo, &ollamaUsageHTTPStub{}, &ollamaUsageTestSettingRepo{}, true)
...
func TestOllamaCloudUsageRefreshSingleflightAndRunnerDeduplicateSharedGroup(t *testing.T) {
	...
	repo := &ollamaUsageTestRepo{
		ollamaUsageTestAccountStore: &ollamaUsageTestAccountStore{accounts: map[int64]*Account{first.ID: first, second.ID: second}},
...
func TestOllamaCloudUsageRefreshRejectsGroupChangeBeforeUpstreamRequest(t *testing.T) {
	...
	base := newOllamaUsageTestRepo(map[int64]*Account{account.ID: account})
	...
	svc := NewOllamaCloudUsageService(repo, upstream, NewSettingService(&ollamaUsageTestSettingRepo{}, nil), ollamaUsageTestEncryptor{}, true)
	t.Cleanup(svc.Stop)
...
func TestOllamaCloudUsageRefreshUsesFixedURLCookieAndNoRedirects(t *testing.T) {
	...
	repo := newOllamaUsageTestRepo(map[int64]*Account{8: account})
	upstream := &ollamaUsageHTTPStub{body: ollamaUsageFixture(t)}
	svc := newOllamaUsageTestService(t, repo, upstream, &ollamaUsageTestSettingRepo{}, true)
...
func TestOllamaCloudUsageRefreshUsesHydratedProxyIdentity(t *testing.T) {
	...
	repo := newOllamaUsageTestRepo(map[int64]*Account{13: account})
	upstream := &ollamaUsageHTTPStub{body: ollamaUsageFixture(t)}
	svc := newOllamaUsageTestService(t, repo, upstream, &ollamaUsageTestSettingRepo{}, true)
...
func TestOllamaCloudUsageRedirectAndBodyLimitArePersistedSafely(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			account.Extra[OllamaCloudUsageSessionExtraKey] = "cipher:wos-session=secret"
			repo := newOllamaUsageTestRepo(map[int64]*Account{9: account})
			svc := newOllamaUsageTestService(t, repo, &ollamaUsageHTTPStub{status: test.status, body: test.body}, &ollamaUsageTestSettingRepo{}, true)
...
func TestOllamaCloudUsageRefreshRejectsIdentityChange(t *testing.T) {
	...
	account.Extra[OllamaCloudUsageSessionExtraKey] = "cipher:wos-session=secret"
	repo := newOllamaUsageTestRepo(map[int64]*Account{10: account})
	repo.beforeSnapshot = func() { account.Credentials["api_key"] = "rotated" }
	svc := newOllamaUsageTestService(t, repo, &ollamaUsageHTTPStub{body: ollamaUsageFixture(t)}, &ollamaUsageTestSettingRepo{}, true)
...
func TestOllamaCloudUsageRunnerHonorsLeaderLockAndBackoff(t *testing.T) {
	...
	repo := newOllamaUsageTestRepo(map[int64]*Account{11: account})
	upstream := &ollamaUsageHTTPStub{body: ollamaUsageFixture(t)}
	settingsRepo := &ollamaUsageTestSettingRepo{values: map[string]string{
		SettingKeyOllamaCloudUsageSettings: `{"enabled":true,"interval_minutes":60}`,
	}}
...
func TestOllamaCloudUsageRunnerDisablesAutoRefreshAfterUnpersistableIdentityError(t *testing.T) {
	...
	account.Proxy = nil
	repo := newOllamaUsageTestRepo(map[int64]*Account{14: account})
	settingsRepo := &ollamaUsageTestSettingRepo{values: map[string]string{
		SettingKeyOllamaCloudUsageSettings: `{"enabled":true,"interval_minutes":60}`,
	}}
	upstream := &ollamaUsageHTTPStub{body: ollamaUsageFixture(t)}
...
func TestOllamaCloudUsageRunnerIdentityChangePreservesOldGroupAndDoesNotLoop(t *testing.T) {
	...
	repo := &ollamaUsageTestRepo{
		ollamaUsageTestAccountStore: &ollamaUsageTestAccountStore{accounts: map[int64]*Account{
...
func TestOllamaCloudUsageSingleflightConcurrencyAndRunnerSwitches(t *testing.T) {
	...
	repo := newOllamaUsageTestRepo(accounts)
