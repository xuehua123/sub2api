...
func TestCalculateOpenAIRecordUsageCostWebSearchPerCall(t *testing.T) {
	...
	cost, _, err := svc.calculateOpenAIRecordUsageCostWithBillingModel(context.Background(), result, apiKey, []string{"gpt-5.6-sol"}, 3.0, 1.0, 1.0, 2.0, UsageTokens{}, "", boolPtr(false))
	...
	cost, _, err = svc.calculateOpenAIRecordUsageCostWithBillingModel(context.Background(), result, apiKey, []string{"gpt-5.6-sol"}, 1.0, 1.0, 1.0, 1.0, UsageTokens{}, "", boolPtr(false))
	require.NoError(t, err)
	...
	_, _, err = svc.calculateOpenAIRecordUsageCostWithBillingModel(context.Background(), result, apiKey, []string{"gpt-5.6-sol"}, 1.0, 1.0, 1.0, 1.0, UsageTokens{InputTokens: 10}, "", boolPtr(false))
	require.Error(t, err)
}
