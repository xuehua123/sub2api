...
func TestCalculateOpenAIRecordUsageCostWebSearchPerCall(t *testing.T) {
	...
	cost, _, err := svc.calculateOpenAIRecordUsageCostWithBillingModel(context.Background(), result, apiKey, []string{"gpt-5.6-sol"}, 3.0, 1.0, 1.0, 2.0, UsageTokens{}, "", boolPtr(false), time.Time{})
	...
	cost, _, err = svc.calculateOpenAIRecordUsageCostWithBillingModel(context.Background(), result, apiKey, []string{"gpt-5.6-sol"}, 1.0, 1.0, 1.0, 1.0, UsageTokens{}, "", boolPtr(false), time.Time{})
	require.NoError(t, err)
	...
	_, _, err = svc.calculateOpenAIRecordUsageCostWithBillingModel(context.Background(), result, apiKey, []string{"gpt-5.6-sol"}, 1.0, 1.0, 1.0, 1.0, UsageTokens{InputTokens: 10}, "", boolPtr(false), time.Time{})
	require.Error(t, err)
}

func TestRecordUsageWebSearchEntitlementAttribution(t *testing.T) {
	...
	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		Result:                     &OpenAIForwardResult{RequestID: "search_entitlement", Model: "gpt-5.6-sol", UpstreamModel: "gpt-5.6-sol", WebSearchCalls: 1},
		APIKey:                     &APIKey{ID: 100, AccessSource: APIKeyAccessSourceEntitlement, GroupID: &groupID, Group: &Group{ID: groupID, Platform: PlatformOpenAI, SubscriptionType: SubscriptionTypeStandard, RateMultiplier: 1}},
		User:                       &User{ID: 200},
		Account:                    &Account{ID: 300, Platform: PlatformOpenAI},
	...
	require.InDelta(t, 0.01, billingRepo.lastCmd.SubscriptionCost, 1e-12)
...
func TestRecordUsageWebSearchBalanceBilling(t *testing.T) {
	usageRepo := &openAIRecordUsageLogRepoStub{inserted: true}
	billingRepo := &openAIRecordUsageBillingRepoStub{result: &UsageBillingApplyResult{Applied: true}}
	svc := newOpenAIRecordUsageServiceWithBillingRepoForTest(usageRepo, billingRepo, &openAIRecordUsageUserRepoStub{}, &openAIRecordUsageSubRepoStub{}, nil)
	groupID := int64(89)
	...
	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		Result:  &OpenAIForwardResult{RequestID: "search_balance", Model: "gpt-5.6-sol", UpstreamModel: "gpt-5.6-sol", WebSearchCalls: 1},
		APIKey:  &APIKey{ID: 101, AccessSource: APIKeyAccessSourceBalance, GroupID: &groupID, Group: &Group{ID: groupID, Platform: PlatformOpenAI, SubscriptionType: SubscriptionTypeStandard, RateMultiplier: 1, WebSearchPricePerCall: &override}},
		User:    &User{ID: 201},
	...
	require.NotNil(t, usageRepo.lastLog)
	require.Nil(t, usageRepo.lastLog.EntitlementID)
	require.NotNil(t, usageRepo.lastLog.BillingMode)
	require.Equal(t, string(BillingModePerRequest), *usageRepo.lastLog.BillingMode)
}
