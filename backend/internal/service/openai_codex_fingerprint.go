...
// codexFingerprintIDsContextKey stores the current attempt's fingerprint set
// on the Gin context. Forwarding code reuses the context across account
// failover attempts, so every consumer must validate the account and API-key
// ownership before applying the value.
...
func stagedCodexFingerprintIDs(c *gin.Context, account *Account) *codexFingerprintIDs {
	...
	if !ok || !codexFingerprintIDsBelongToAttempt(ids, account, getAPIKeyIDFromContext(c)) {
		return nil
	}
	return ids
}
...
func resolveSessionFingerprintSessionID(seed string, apiKeyID int64) string {
	base := resolveConvergedSessionID(seed)
	if base == "" {
		return ""
	}
...
func newCodexFingerprintTurnIDs(base *codexFingerprintIDs) *codexFingerprintIDs {
	...
	if base.mode != codexFingerprintDevice && base.mode != codexFingerprintOff {
		...
		next.originalBodySessionID = ""
		next.originalBodySessionIDCaptured = false
	}
	return &next
}
...
func resolveCodexFingerprintIDs(account *Account, clientSessionID string, mode codexFingerprintMode, apiKeyID int64) *codexFingerprintIDs {
	...
	case codexFingerprintSession:
		ids.sessionID = resolveSessionFingerprintSessionID(seed, apiKeyID)
