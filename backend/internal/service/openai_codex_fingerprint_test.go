...
func TestCodexFingerprintIDsBelongToAccount(t *testing.T) {
	account := newTestOAuthAccount(1, map[string]any{codexFingerprintModeExtraKey: "session"})
	other := newTestOAuthAccount(2, map[string]any{codexFingerprintModeExtraKey: "session"})
	ids := resolveCodexFingerprintIDsFromRequest(account, nil)
...
func TestNewCodexFingerprintTurnIDsPreservesConnectionIdentity(t *testing.T) {
	...
	base := resolveCodexFingerprintIDsFromRequest(account, http.Header{"session-id": []string{"client-session"}})
	require.NotNil(t, base)

	next := newCodexFingerprintTurnIDs(base)
	require.NotNil(t, next)
	assert.Equal(t, base.accountID, next.accountID)
	assert.Equal(t, base.mode, next.mode)
	assert.Equal(t, base.installationID, next.installationID)
	assert.Equal(t, base.sessionID, next.sessionID)
	assert.Equal(t, base.threadID, next.threadID)
	assert.Equal(t, base.windowID, next.windowID)
	assert.NotEqual(t, base.turnID, next.turnID)
}

func TestEnsureCodexFingerprintAttemptIDsReusesConnectionAndRotatesTurns(t *testing.T) {
	gin.SetMode(gin.TestMode)
	c, _ := gin.CreateTestContext(httptest.NewRecorder())
	c.Request = httptest.NewRequest(http.MethodGet, "/v1/responses", nil)
	c.Request.Header.Set("session-id", "client-session")
	account := newTestOAuthAccount(12, map[string]any{codexFingerprintModeExtraKey: "session"})
	...
	next := advanceCodexFingerprintTurn(c, account)
	require.NotNil(t, next)
...
func TestResolveSessionFingerprintSessionIDIsolatesAPIKeys(t *testing.T) {
	...
	assert.Equal(t, resolveConvergedInstallationID(account, testCodexFingerprintSeed), idsA.installationID)
	...
	assert.Equal(t, isolateOpenAISessionID(11, resolveConvergedSessionID(testCodexFingerprintSeed)), idsA.sessionID)
	assert.Equal(t, isolateOpenAISessionID(22, resolveConvergedSessionID(testCodexFingerprintSeed)), idsB.sessionID)
}
...
func TestResolveFullFingerprintSessionIDIsAccountScoped(t *testing.T) {
	...
	require.NotNil(t, idsA)
	require.NotNil(t, idsB)
	assert.Equal(t, resolveConvergedSessionID(testCodexFingerprintSeed), idsA.sessionID)
...
func TestBuildOpenAIWSHeadersSessionModeKeepsAPIKeyIsolation(t *testing.T) {
	...
	headersA := build(11)
	headersB := build(22)
	assert.Equal(t, resolveConvergedInstallationID(account, testCodexFingerprintSeed), headersA.Get("x-codex-installation-id"))
	assert.Equal(t, headersA.Get("x-codex-installation-id"), headersB.Get("x-codex-installation-id"))
	assert.Equal(t, isolateOpenAISessionID(11, resolveConvergedSessionID(testCodexFingerprintSeed)), headersA.Get("session_id"))
	assert.Equal(t, isolateOpenAISessionID(22, resolveConvergedSessionID(testCodexFingerprintSeed)), headersB.Get("session_id"))
...
func TestEnsureCodexFingerprintAttemptIDsDoesNotReuseAnotherAccount(t *testing.T) {
	...
	c, _ := gin.CreateTestContext(httptest.NewRecorder())
	c.Request = httptest.NewRequest(http.MethodGet, "/v1/responses", nil)
	firstAccount := newTestOAuthAccount(21, map[string]any{codexFingerprintModeExtraKey: "session"})
	secondAccount := newTestOAuthAccount(22, map[string]any{codexFingerprintModeExtraKey: "session"})
	secondAccount.Extra[codexFingerprintSeedExtraKey] = "11111111-2222-4333-8444-555555555555"

	first := ensureCodexFingerprintAttemptIDs(c, firstAccount)
	second := ensureCodexFingerprintAttemptIDs(c, secondAccount)
	require.NotNil(t, first)
	require.NotNil(t, second)
	assert.Equal(t, firstAccount.ID, first.accountID)
	assert.Equal(t, secondAccount.ID, second.accountID)
	assert.NotEqual(t, first.installationID, second.installationID)
}

func TestBuildOpenAIWSHeadersUsesFingerprintTurnIDs(t *testing.T) {
	gin.SetMode(gin.TestMode)
	c, _ := gin.CreateTestContext(httptest.NewRecorder())
	c.Request = httptest.NewRequest(http.MethodGet, "/v1/responses", nil)
	c.Request.Header.Set("session-id", "client-session")
	...
	require.NoError(t, err)
	ids := ensureCodexFingerprintAttemptIDs(c, account)
	require.NotNil(t, ids)
...
func TestApplyCodexFingerprintJSONBody(t *testing.T) {
	account := newTestOAuthAccount(1, map[string]any{codexFingerprintModeExtraKey: "session"})
	clientHeaders := http.Header{}
	clientHeaders.Set("session-id", "client-session-json")
	ids := resolveCodexFingerprintIDsFromRequest(account, clientHeaders)
	require.NotNil(t, ids)

	original := []byte(`{"model":"gpt-5.6","client_metadata":{"opaque":"kept"}}`)
	updated, changed, err := applyCodexFingerprintJSONBody(original, ids)
	require.NoError(t, err)
	require.True(t, changed)
	assert.Equal(t, "kept", gjson.GetBytes(updated, "client_metadata.opaque").String())
	assert.Equal(t, ids.installationID, gjson.GetBytes(updated, "client_metadata.x-codex-installation-id").String())
	assert.Equal(t, ids.sessionID, gjson.GetBytes(updated, "client_metadata.session_id").String())
	assert.Equal(t, ids.threadID, gjson.GetBytes(updated, "client_metadata.thread_id").String())
	assert.Equal(t, ids.turnID, gjson.GetBytes(updated, "client_metadata.turn_id").String())
}

func TestApplyCodexFingerprintJSONBodyOffKeepsBytes(t *testing.T) {
	original := []byte(" {\n  \"model\": \"gpt-5.6\"\n} ")
	updated, changed, err := applyCodexFingerprintJSONBody(original, nil)
	require.NoError(t, err)
...
func TestRewriteCodexTurnMetadataFields_NullInitializesObject(t *testing.T) {
	h := http.Header{}
	h.Set("x-codex-turn-metadata", "null")
	rewriteCodexTurnMetadataFields(h, map[string]any{"installation_id": "stable"})
	var metadata map[string]any
	require.NoError(t, json.Unmarshal([]byte(h.Get("x-codex-turn-metadata")), &metadata))
	assert.Equal(t, "stable", metadata["installation_id"])
}

func TestRewriteClientMetadataEmbeddedTurnMetadata_NullInitializesObject(t *testing.T) {
	metadata := map[string]any{"x-codex-turn-metadata": "null"}
	rewriteClientMetadataEmbeddedTurnMetadata(metadata, map[string]any{"thread_id": "stable"})
	var embedded map[string]any
	raw, ok := metadata["x-codex-turn-metadata"].(string)
	require.True(t, ok)
	err := json.Unmarshal([]byte(raw), &embedded)
	require.NoError(t, err)
	assert.Equal(t, "stable", embedded["thread_id"])
}
...
func TestApplyCodexFingerprintHeaders_SessionMode(t *testing.T) {
	...
	convergedSession := resolveSessionFingerprintSessionID(seed, 0)
...
func TestApplyCodexFingerprintPromptCacheKey_MapRawEquivalence(t *testing.T) {
	for _, mode := range []codexFingerprintMode{codexFingerprintSession, codexFingerprintFull} {
		t.Run(string(mode)+"/default", func(t *testing.T) {
			...
			ids := resolveCodexFingerprintIDs(account, "header-session", mode, 0)
			require.NotNil(t, ids)
	...
	t.Run("explicit override", func(t *testing.T) {
		...
		ids := resolveCodexFingerprintIDs(account, "header-session", codexFingerprintSession, 0)
		require.NotNil(t, ids)
...
func TestApplyCodexFingerprintPromptCacheKey_Negatives(t *testing.T) {
	...
	sessionIDs := resolveCodexFingerprintIDs(sessionAccount, "header-session", codexFingerprintSession, 0)
	require.NotNil(t, sessionIDs)
	deviceAccount := newTestOAuthAccount(4311, map[string]any{codexFingerprintModeExtraKey: "device"})
	deviceIDs := resolveCodexFingerprintIDs(deviceAccount, "header-session", codexFingerprintDevice, 0)
...
func TestApplyCodexFingerprintClientMetadataRaw_MatchesMapVariant(t *testing.T) {
	...
	for _, mode := range []codexFingerprintMode{codexFingerprintDevice, codexFingerprintSession, codexFingerprintFull} {
		...
		ids := resolveCodexFingerprintIDs(account, "client-sess-raw", mode, 0)
		require.NotNil(t, ids)
...
func TestApplyCodexFingerprintClientMetadataRaw_PreservesUnrelatedFields(t *testing.T) {
	...
	ids := resolveCodexFingerprintIDs(account, "client-sess-preserve", codexFingerprintSession, 0)
	require.NotNil(t, ids)
...
func TestStageCodexFingerprintIDs_NilOverwritesPreviousAccount(t *testing.T) {
	...
	idsA := resolveCodexFingerprintIDs(accountA, "sess-x", codexFingerprintSession, 0)
	require.NotNil(t, idsA)
...
func TestApplyStagedCodexFingerprintRejectsDifferentOAuthAccount(t *testing.T) {
	...
	idsA := resolveCodexFingerprintIDs(accountA, "sess-a", codexFingerprintSession, 0)
	require.NotNil(t, idsA)
	stageCodexFingerprintIDs(c, idsA)
...
func TestApplyStagedCodexFingerprintHeaders_SkipsNonOAuthAccount(t *testing.T) {
	c := newFingerprintStageTestContext(t)
	oauthIDs := resolveCodexFingerprintIDs(newTestOAuthAccount(1003, map[string]any{codexFingerprintModeExtraKey: "session"}), "sess-y", codexFingerprintSession, 0)
...
func TestApplyCodexFingerprintClientMetadataRaw_NonObjectBodyUntouched(t *testing.T) {
	...
	ids := resolveCodexFingerprintIDs(account, "client-sess-nonobj", codexFingerprintSession, 0)
	require.NotNil(t, ids)
