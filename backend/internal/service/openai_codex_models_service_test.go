...
// Scenario: upstream /v1/models conversion must expose exactly the same Fast
// capability set that the billing policy can price.
func TestConvertOpenAIModelListToCodexManifestAlignsPriorityTierWithBillingPolicy(t *testing.T) {
	t.Parallel()
	...
	models := decodeCodexManifestModels(t, convertOpenAIModelListToCodexManifest([]byte(upstreamBody)))
	require.Len(t, models, 5)

	for _, model := range models {
		slug, ok := model["slug"].(string)
		require.True(t, ok)
		normalized := normalizeKnownOpenAICodexModel(slug)
		billingSupportsFast := openAIModelFastPricingRatio(normalized) > 0
...
func TestCompleteAPIKeyCodexModelsManifestForClientDropsUnknownProviderTierAndPreservesOtherMetadata(t *testing.T) {
	t.Parallel()

	svc := &OpenAIGatewayService{}
	manifest := &CodexModelsManifest{
		Body: []byte(`{"models":[{"slug":"grok-4.6","description":"Provider supplied","service_tiers":[{"id":"provider-priority","name":"Provider Fast","description":"Provider supplied tier."}],"default_service_tier":"provider-priority","model_messages":{"auto_review":{"enabled":true}},"truncation_policy":{"mode":"tokens"},"unknown":{"kept":true}}],"metadata":{"source":"upstream"}}`),
	}
	account := newCodexModelsAPIKeyTestAccount("https://upstream.example/v1")
	...
	require.Nil(t, models[0]["default_service_tier"])
	require.Equal(t, []any{"text"}, models[0]["input_modalities"])
	require.Equal(t, []string{"low", "medium", "high", "xhigh"}, effortsFromManifestModel(t, models[0]))
...
func TestCompleteAPIKeyCodexModelsManifestForClientCanonicalizesProviderServiceTiers(t *testing.T) {
	t.Parallel()

	svc := &OpenAIGatewayService{}
	manifest := &CodexModelsManifest{
		Body: []byte(`{"models":[{"slug":"grok-4.6","service_tiers":[
			{"id":" FAST ","name":"Provider Fast","description":"Provider supplied tier.","provider_meta":{"kept":true}},
			{"id":"priority","name":"Duplicate Fast","description":"Must be removed."},
			{"id":"flex","name":"Flex","description":"Flexible processing."},
			{"id":"auto","name":"Auto","description":"Automatic tier."},
			{"id":"default","name":"Default","description":"Default tier."},
			{"id":"scale","name":"Scale","description":"Scale tier."},
			{"id":"provider-priority","name":"Unknown","description":"Must be removed."},
			{"id":42,"name":"Invalid","description":"Must be removed."}
	...
	models := decodeCodexManifestModels(t, manifest.Body)
	require.Len(t, models, 1)
	tiers, ok := models[0]["service_tiers"].([]any)
	require.True(t, ok)
	require.Len(t, tiers, 5)
	firstTier, ok := tiers[0].(map[string]any)
	require.True(t, ok)
	require.Equal(t, "priority", firstTier["id"])
	require.Equal(t, "Provider Fast", firstTier["name"])
	require.Equal(t, map[string]any{"kept": true}, firstTier["provider_meta"])
	require.Equal(t, "priority", models[0]["default_service_tier"])

	wantIDs := []string{"priority", "flex", "auto", "default", "scale"}
	for i, wantID := range wantIDs {
		tier, ok := tiers[i].(map[string]any)
		require.True(t, ok)
		require.Equal(t, wantID, tier["id"])
		requestBody, err := json.Marshal(map[string]any{"service_tier": wantID})
		require.NoError(t, err)
		validated, err := ValidateOpenAIServiceTierField(requestBody)
		require.NoError(t, err)
		require.Equal(t, wantID, validated)
		_, billable := serviceTierCostRank(wantID)
		require.True(t, billable, "advertised tier %q must have defined billing semantics", wantID)
	}
	require.Equal(t, codexModelsManifestBodyETag(manifest.Body), manifest.ETag)
}
