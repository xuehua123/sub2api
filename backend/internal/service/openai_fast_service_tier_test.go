...
func TestOpenAIFastBillingMultiplier_2xAnd25x(t *testing.T) {
	...
	// Keep this ratio test below the long-context threshold. GPT-5.4 priority
	// intentionally falls back to standard long-context rates above 272K and is
	// covered separately by the long-context policy tests.
...
func TestOpenAIFastBilling_FastMultiplierOverridesEnforcedRatio(t *testing.T) {
	...
	standard := svc.computeTokenBreakdown("gpt-5.5", pricing, tokens, 1, "", false)
	fast := svc.computeTokenBreakdown("gpt-5.5", pricing, tokens, 1, "fast", false)
	priority := svc.computeTokenBreakdown("gpt-5.5", pricing, tokens, 1, "priority", false)
	...
	enforced := svc.computeTokenBreakdown("gpt-5.5", &withoutOverride, tokens, 1, "fast", false)
