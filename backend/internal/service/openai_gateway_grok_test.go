...
func TestAccountTestServiceGrokPoolModePaymentRequiredDoesNotUnscheduleWholePool(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusPaymentRequired,
		Header:     http.Header{"Content-Type": []string{"application/json"}},
		Body:       io.NopCloser(strings.NewReader(`{"code":"pool-key-spending-limit"}`)),
	}}
	svc := &AccountTestService{
		accountRepo:  repo,
		httpUpstream: upstream,
...
func TestForwardGrokResponsesMonitorProbe400FailsOver(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	c, _ := gin.CreateTestContext(recorder)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/responses", bytes.NewReader(body))
	c.Set("api_key", &APIKey{ID: 5901})

	account := grokChatBridgeTestAccount(590)
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusBadRequest,
		Header:     http.Header{"Content-Type": []string{"application/json"}},
		Body:       io.NopCloser(strings.NewReader(`{"error":{"message":"bad monitor request"}}`)),
	}}
	svc := &OpenAIGatewayService{
		httpUpstream:      upstream,
		grokTokenProvider: NewGrokTokenProvider(repo, nil),
	...
	result, err := svc.forwardGrokResponses(WithChannelMonitorProbe(context.Background()), c, account, body, "grok", false, time.Now())
	require.Error(t, err)
	require.Nil(t, result)
	var failoverErr *UpstreamFailoverError
	require.ErrorAs(t, err, &failoverErr)
...
func TestDescribeGrokComposerImageMonitorProbe400FailsOver(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	account := grokChatBridgeTestAccount(591)
	repo := &grokQuotaAccountRepo{mockAccountRepoForPlatform: &mockAccountRepoForPlatform{
		accountsByID: map[int64]*Account{account.ID: account},
	}}
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusBadRequest,
		Header:     http.Header{"Content-Type": []string{"application/json"}},
		Body:       io.NopCloser(strings.NewReader(`{"error":{"message":"bad image monitor request"}}`)),
	}}
	svc := &OpenAIGatewayService{
		httpUpstream: upstream,
		accountRepo:  repo,
	...
	description, usage, err := svc.describeGrokComposerImage(
		WithChannelMonitorProbe(context.Background()), c, account, "access-token", "data:image/png;base64,QUJD", 1,
	)
	require.Error(t, err)
...
func TestHandleGrokAccountUpstreamErrorPoolModeHonorsExplicitPolicies(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			shouldDisable := svc.handleGrokAccountUpstreamError(context.Background(), account, tt.status, http.Header{}, tt.body)
