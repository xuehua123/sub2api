...
func normalizeRequestedReasoningEffort(raw string) string {
...
// later persist "xhigh"; "none" is preserved for non-reasoning catalog entries
// even when a compatible upstream later strips it from the forwarded body.
func CanonicalRequestedReasoningEffort(body []byte, modelCandidates ...string) *string {
	if raw := explicitRequestedReasoningEffortFromBody(body); raw != "" {
		canonical := normalizeRequestedReasoningEffort(raw)
		if canonical == "" {
			return nil
		}
		return &canonical
	}
	// The body is the current request/frame and must win over fallback candidates
	// retained from an earlier session turn. This matters for multi-turn WS
	// clients that explicitly switch from e.g. gpt-5.6-low to gpt-5.6-high.
...
func canonicalReasoningEffortFromModelSuffix(model string) string {
	...
	return normalizeRequestedReasoningEffort(parts[len(parts)-1])
}
