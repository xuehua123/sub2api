...
func (s *OpenAIGatewayService) handleStreamingResponseWithReasoning(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, startTime time.Time, originalModel, mappedModel, reasoningEffort string) (*openaiStreamingResult, error) {
	...
	flushBuffered := func() error {
		hadBufferedData := pendingBytes() > 0
		if firstOutputStage != nil && !firstOutputStage.closed {
...
func openAIResponsesCompletedEventIsEmpty(data []byte, usage *OpenAIUsage) bool {
	...
	for _, path := range []string{"usage", "response.usage"} {
		if usageValue := gjson.GetBytes(data, path); usageValue.Exists() {
			// An empty usage object is emitted by some OpenAI-compatible upstreams
			// on a silent completed event and must not suppress failover.
			if !usageValue.IsObject() || len(usageValue.Map()) > 0 {
				return false
			}
		}
	}
	...
	for _, path := range []string{"output", "response.output"} {
		if output := gjson.GetBytes(data, path); output.Exists() && output.IsArray() && len(output.Array()) > 0 {
			return false
		}
	}
	return true
	...
	if terminalOK && (terminalType == "response.failed" || terminalType == "error") {
		...
		if openAIStreamFailureStatus(terminalPayload, msg) == http.StatusTooManyRequests {
			return nil, s.newOpenAIStreamFailoverError(
				c, account, false, resp.Header.Get("x-request-id"), terminalPayload, msg, resp.Header,
			)
		}
