...
func TestForwardResponses_ForceChatCompletionsOmitsNoneReasoningEffort(t *testing.T) {
	...
	requestedReasoningEffort := CanonicalRequestedReasoningEffort(body, "company-coding-model")
	require.NotNil(t, requestedReasoningEffort)
	require.Equal(t, "none", *requestedReasoningEffort)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/responses", bytes.NewReader(body))
	c.Request.Header.Set("Content-Type", "application/json")
