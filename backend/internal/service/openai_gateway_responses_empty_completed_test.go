...
func TestOpenAIResponsesEmptyCompletedNonStreamingPassthroughFailsOver(t *testing.T) {
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		...
		Header: http.Header{
			...
			"x-request-id": []string{"rid-empty-buffered"},
		},
		Body: io.NopCloser(strings.NewReader(
			"data: {\"type\":\"response.created\",\"response\":{\"id\":\"resp_empty_buffered\",\"status\":\"in_progress\"}}\n\n" +
				"data: {\"type\":\"response.completed\",\"response\":{\"id\":\"resp_empty_buffered\",\"status\":\"completed\",\"output\":[]}}\n\n",
		)),
	}}
	svc := newOpenAIImageGenerationControlTestService(upstream)
	c, recorder := newOpenAIImageGenerationControlTestContext(true, "codex_cli_rs/0.144.1")
	...
	require.Equal(t, "rid-empty-buffered", failoverErr.ResponseHeaders.Get("x-request-id"))
	require.Empty(t, recorder.Body.String())
}

func TestOpenAIResponsesNonStreamingPassthroughDeltaOutputSucceeds(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		...
		Body: io.NopCloser(strings.NewReader(
			"data: {\"type\":\"response.output_text.delta\",\"delta\":\"hello\"}\n\n" +
				"data: {\"type\":\"response.completed\",\"response\":{\"id\":\"resp_delta_buffered\",\"status\":\"completed\",\"output\":[]}}\n\n",
		)),
	}}
	svc := newOpenAIImageGenerationControlTestService(upstream)
	c, recorder := newOpenAIImageGenerationControlTestContext(true, "codex_cli_rs/0.144.1")
...
func TestOpenAIResponsesNonStreamingPassthroughEarlierUsageSucceeds(t *testing.T) {
	gin.SetMode(gin.TestMode)
...
func TestOpenAIResponsesCompletedEventIsEmpty(t *testing.T) {
	cases := []struct {
		...
		{
			name: "completed with empty usage object",
		...
		{
			name: "extracted response with output item",
			data: `{"id":"r1","status":"completed","output":[{"type":"message","id":"msg_1"}]}`,
			want: false,
		},
		{
