...
func TestOpenAIBuildUpstreamRequestCanonicalizesCodexSessionIDForOAuth(t *testing.T) {
	...
	cHTTP := newContext()
	expectedHTTP := isolateOpenAIUpstreamSessionID(42, codexAccountIdentitySource(cHTTP, account), "codex-session-A")
	reqHTTP, err := svc.buildUpstreamRequest(cHTTP.Request.Context(), cHTTP, account, body, "token", false, "", true)
	require.NoError(t, err)
	require.Equal(t, expectedHTTP, reqHTTP.Header.Get("session_id"))
	require.Empty(t, reqHTTP.Header.Get("session-id"))

	cPassthrough := newContext()
	expectedPassthrough := isolateOpenAIUpstreamSessionID(42, codexAccountIdentitySource(cPassthrough, account), "codex-session-A")
	...
	require.NoError(t, err)
	require.Equal(t, expectedPassthrough, reqPassthrough.Header.Get("session_id"))
...
func TestHandleNonStreamingResponse_ResponseFailedRateLimitUsesPoolRetryPolicy(t *testing.T) {
	gin.SetMode(gin.TestMode)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/responses", nil)
	...
	resp := &http.Response{
		...
		Header: http.Header{
			...
			"Retry-After":  []string{"1"},
			"x-request-id": []string{"rid-buffered-rate-limit"},
		},
		Body: io.NopCloser(strings.NewReader(strings.Join([]string{
			`event: response.failed`,
