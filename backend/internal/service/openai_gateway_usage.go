...
func (s *OpenAIGatewayService) RecordUsage(ctx context.Context, input *OpenAIRecordUsageInput) error {
	...
	var baselineBillingModel string
	cost, baselineBillingModel, err = s.calculateOpenAIRecordUsageCostWithBillingModel(
		ctx,
		result,
		apiKey,
		billingModels,
	...
	if responseModel := responseModelBillingDeclaration(
		...
		if identified, responseChannelPriced := s.hasIdentifiedOpenAIResponsePricing(ctx, responseModel, apiKey); identified {
			...
			responseCost, _, responseErr := s.calculateOpenAIRecordUsageCostWithBillingModel(
			...
			// 基线定价源必须跟随真正算出成本的候选。首候选可能是上游账号的 opaque alias，
			// 定价循环会跳过它并由后续 mapped/requested model 命中 Group/Channel 价；若仍按
			// 首候选判断，会把显式配置误认为全局价，进而允许 response_model 静默降价。
...
func (s *OpenAIGatewayService) hasIdentifiedOpenAIResponsePricing(ctx context.Context, model string, apiKey *APIKey) (identified bool, channelPriced bool) {
	...
	if resolved := s.resolveOpenAIChannelPricing(ctx, model, apiKey); resolved != nil {
		// Response-model billing is an ordinary token-pricing admission check.
		// A channel explicitly configured for per-request/image billing must not
		// turn a token response into a media/request charge.
...
// calculateOpenAIRecordUsageCost preserves the legacy cost-calculation entry
// point while the detailed implementation also returns the resolved billing model.
func (s *OpenAIGatewayService) calculateOpenAIRecordUsageCost(
	...
	cost, _, err := s.calculateOpenAIRecordUsageCostWithBillingModel(
...
// calculateOpenAIRecordUsageCostWithBillingModel additionally returns the exact
// candidate that produced the cost. This is security-sensitive metadata for the
// response_model adoption guard: the first candidate is not necessarily the one
// that resolved successfully.
func (s *OpenAIGatewayService) calculateOpenAIRecordUsageCostWithBillingModel(
	ctx context.Context,
	result *OpenAIForwardResult,
	apiKey *APIKey,
	billingModels []string,
