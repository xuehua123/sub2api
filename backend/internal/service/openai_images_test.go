...
func TestOpenAIGatewayServiceForwardImages_OAuthEditsRewritesTrustedImageURLsToDataURLs(t *testing.T) {
	...
	require.NoError(t, err)
	require.NotNil(t, result)
	require.True(t, strings.HasPrefix(gjson.GetBytes(upstream.lastBody, "images.0.image_url").String(), "data:image/jpeg;base64,"))
	require.True(t, strings.HasSuffix(upstream.lastReq.URL.Path, "/images/edits"))
}
