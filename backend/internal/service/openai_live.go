...
func (s *OpenAIGatewayService) CreateLiveCall(
	...
	for attempt := 0; attempt <= 3; attempt++ {
		...
		record := &LiveCallRecord{
			...
			EntitlementID:              liveGroupID(identity.EntitlementID),
			EntitlementBalanceFallback: identity.EntitlementBalanceFallback,
			LeaseID:                    leaseID,
			Model:                      model,
			CreatedAt:                  now,
...
func (s *OpenAIGatewayService) finalizeLiveCall(record *LiveCallRecord) {
	...
	if record.SubscriptionID > 0 || record.EntitlementID > 0 {
	...
	usageLog := &UsageLog{
		...
		EntitlementID:    liveOptionalID(record.EntitlementID),
		RateMultiplier:   1,
	...
	usageLog.SetBillingSource(ResolveUsageBillingSource(
		...
		record.EntitlementBalanceFallback,
	))
	_ = writeUsageLogBestEffort(context.Background(), s.usageLogRepo, usageLog, "service.openai_live")
}
