...
func TestOpenAIGatewayService_Forward_WSv2_TurnStateBoundToExecutionScope(t *testing.T) {
	...
	_, turnStateOnLegacy := store.GetSessionTurnState(groupID, 21, account.ID, legacyHash)
	require.False(t, turnStateOnLegacy, "turn state 不得绑定到按 session_id 算出的会话哈希")
	turnState, turnStateOnScope := store.GetSessionTurnState(groupID, 21, account.ID, scope)
...
func TestOpenAIGatewayService_Forward_WSv2_ExecutionScopeUsesOriginalIdentity(t *testing.T) {
	...
	forward := func(sessionID, body string) (*gin.Context, []byte) {
		requestPool := newOpenAIWSConnPool(cfg)
		requestPool.setClientDialerForTest(&openAIWSCaptureDialer{conn: &openAIWSCaptureConn{events: [][]byte{completed(sessionID)}}, handshake: handshake})
		defer requestPool.Close()
		svc.openaiWSPool = requestPool
		rec := httptest.NewRecorder()
		c, _ := gin.CreateTestContext(rec)
		c.Request = httptest.NewRequest(http.MethodPost, "/openai/v1/responses", nil)
	...
	_, boundA := stateStore.GetSessionTurnState(groupID, 21, account.ID, scopeA)
	require.True(t, boundA, "会话 A 的 turn state 应落在按原始 session_id 算出的作用域")
	_, boundB := stateStore.GetSessionTurnState(groupID, 21, account.ID, scopeB)
	require.True(t, boundB, "会话 B 的 turn state 应落在按原始 session_id 算出的作用域")

	injected := resolveCodexFingerprintIDs(account, "", codexFingerprintFull, apiKeyID)
	...
	_, boundToInjected := stateStore.GetSessionTurnState(groupID, 21, account.ID, injectedScope)
	...
	_, boundC := stateStore.GetSessionTurnState(groupID, 21, account.ID, scopeC)
