...
func (s *OpenAIGatewayService) proxyOpenAIWSHTTPBridgeTurn(
	...
	upstreamCtx = ContextWithAccountUpstreamPolicy(upstreamCtx, account)
	...
	for scanner.Scan() {
		...
		setStreamElapsedMsOnce(&firstSSEEventMs, turnStart)
		if trimmedData == "[DONE]" {
			sawDone = true
			continue
		}
		...
		if eventType == "error" || eventType == "response.failed" {
			...
			if account.Platform == PlatformGrok {
				...
				if eventType == "error" {
					...
					errMessage = grokWSHTTPBridgeFailureMessage(upstreamMessage)
				}
				// Grok has request- and model-scoped classifications that differ
				// from the generic OpenAI response.failed policy. Resolve those
				// before the shared failover return so account reconciliation and
				// per-model blocking cannot be bypassed by the HTTP bridge.
			...
			if account.Platform != PlatformGrok && eventType == "error" && shouldFailover && !requestScopedCapacity {
		...
		if !clientDisconnected {
			...
			if stageBeforeSemanticOutput && !commitStagedMessages {
				...
				for i, message := range messages {
					...
					if i == len(messages)-1 && isOpenAIWSTerminalEvent(eventType) {
