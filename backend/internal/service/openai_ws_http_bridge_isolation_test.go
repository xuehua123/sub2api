...
func TestOpenAIWSHTTPBridgeSessionIsolationAcrossSameSessionHash(t *testing.T) {
	...
	stateStore.BindSessionTurnState(groupID, 11, account.ID, seedHash, "sentinel-turn-state", time.Hour)
	...
	gotState, ok := stateStore.GetSessionTurnState(groupID, 11, account.ID, seedHash)
	require.True(t, ok)
