...
func TestProxyOpenAIWSHTTPBridgeTurnPreservesRequestedReasoningEffort(t *testing.T) {
	...
	tests := []struct {
		...
		wantForwardedField bool
	}{
		{
			name:               "policy downgrade keeps original request",
			account:            &Account{ID: 5883, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Concurrency: 1},
			payload:            `{"type":"response.create","model":"gpt-5.5","reasoning":{"effort":"low"},"input":"hi"}`,
			requested:          "high",
			wantForwarded:      "low",
			wantForwardedField: true,
		},
		{
			name: "compatible endpoint strips none without losing request",
			account: &Account{
				ID: 5884, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Concurrency: 1,
				Credentials: map[string]any{"base_url": "https://compat.example/v1"},
			},
			payload:   `{"type":"response.create","model":"gpt-5.5","reasoning":{"effort":"none"},"input":"hi"}`,
			requested: "none",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			sse := "data: {\"type\":\"response.completed\",\"response\":{\"id\":\"resp_effort\",\"status\":\"completed\",\"usage\":{\"input_tokens\":1,\"output_tokens\":1}}}\n\n"
			...
			result, err := svc.proxyOpenAIWSHTTPBridgeTurn(
				context.Background(), c, tt.account, "test-token", payload, len(payload),
				"gpt-5.5", &requested, "", "", "", "", 1,
			...
			require.Equal(t, tt.requested, *result.RequestedReasoningEffort)
			forwarded := gjson.GetBytes(upstream.lastBody, "reasoning.effort")
			require.Equal(t, tt.wantForwardedField, forwarded.Exists())
			if tt.wantForwardedField {
				require.Equal(t, tt.wantForwarded, forwarded.String())
				require.NotNil(t, result.ReasoningEffort)
				require.Equal(t, tt.wantForwarded, *result.ReasoningEffort)
			}
		})
	}
}
...
func TestProxyOpenAIWSHTTPBridgeTurnGrokInheritsToolSearchAndPromotesFollowupDiscovery(t *testing.T) {
	...
	_, err := svc.proxyOpenAIWSHTTPBridgeTurn(
		...
		"grok-4.5", nil, "", "", "", "grok-ws-cache", 1, func([]byte) error { return nil },
	)
	require.NoError(t, err)
...
func TestProxyOpenAIWSHTTPBridgeTurnBareErrorUsesAuthoritativeFailed(t *testing.T) {
	...
	result, err := svc.proxyOpenAIWSHTTPBridgeTurn(context.Background(), c, account, "sk-test", payload, len(payload), "gpt-5", nil, "", "", "", "", 2, func(message []byte) error {
		writes = append(writes, append([]byte(nil), message...))
		return nil
	})
...
func TestProxyOpenAIWSHTTPBridgeTurnMarksCyberPolicyForFailureShapes(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := svc.proxyOpenAIWSHTTPBridgeTurn(
				context.Background(), c, account, "sk-test", payload, len(payload),
				"gpt-5", nil, "", "", "", "", 1, func([]byte) error { return nil },
			)
...
func TestOpenAIWSHTTPBridgeRelaysSSEFramesAsWebSocketMessages(t *testing.T) {
	...
	case bridge := <-resultCh:
		...
		require.NotNil(t, bridge.result.FirstSSEEventMs)
		require.NotNil(t, bridge.result.FirstClientFlushMs)
...
func TestProxyOpenAIWSHTTPBridgeTurnPromotesCodexAdditionalToolsForMixedCache(t *testing.T) {
	...
	result, err := svc.proxyOpenAIWSHTTPBridgeTurn(
		context.Background(), c, account, "access-token", payload, len(payload),
		"grok", nil, "", "", "", "isolated-ws-cache-id", 1,
		func(message []byte) error {
			events = append(events, append([]byte(nil), message...))
			return nil
		},
