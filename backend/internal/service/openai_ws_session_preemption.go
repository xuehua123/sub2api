...
func (s *OpenAIGatewayService) BeginOpenAIWSIngressSessionPreemption(
	...
	if preemptedPrevious {
		if stateStore := s.getOpenAIWSStateStore(); stateStore != nil {
			stateStore.DeleteSessionTurnState(preemptGroupID, getAPIKeyIDFromContext(c), account.ID, preemptSessionHash)
...
func (s *OpenAIGatewayService) beginOpenAIWSSessionPreemptContext(
	...
	preempt := func() {
		preemptOnce.Do(func() {
			if stateStore := s.getOpenAIWSStateStore(); stateStore != nil {
				stateStore.DeleteSessionTurnState(key.groupID, key.apiKeyID, account.ID, key.sessionHash)
