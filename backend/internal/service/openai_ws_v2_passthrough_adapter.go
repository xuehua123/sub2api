...
func (m *openAIWSPassthroughUsageMeta) failureModel(fallback string) string {
	_, upstreamModel := m.turnModels(fallback)
...
func (s *OpenAIGatewayService) proxyResponsesWebSocketV2Passthrough(
	...
	if filterReasoningErr != nil {
		return NewOpenAIWSClientCloseError(coderws.StatusPolicyViolation, "invalid websocket reasoning payload", filterReasoningErr)
	}
	firstClientMessage = filteredFirstClientMessage
	...
	if account.IsOpenAIOAuth() {
		if ids := ensureCodexFingerprintAttemptIDs(c, account); ids != nil {
			fingerprinted, changed, fingerprintErr := applyCodexFingerprintJSONBody(firstClientMessage, ids)
			if fingerprintErr != nil {
				return fmt.Errorf("apply OpenAI Codex fingerprint metadata to websocket payload: %w", fingerprintErr)
			}
			if changed {
	...
	policyClientConn := &openAIWSPolicyEnforcingFrameConn{
		...
		filter: func(msgType coderws.MessageType, payload []byte) (out []byte, blocked *OpenAIFastBlockedError, filterErr error) {
			...
			if isResponseCreate {
				...
				if filterReasoningErr != nil {
					return payload, nil, NewOpenAIWSClientCloseError(coderws.StatusPolicyViolation, "invalid websocket reasoning payload", filterReasoningErr)
				}
			...
			if isResponseCreate {
				...
				if hooks != nil && hooks.TransformRequest != nil {
					...
					if len(bytes.TrimSpace(transformed)) == 0 || !gjson.ValidBytes(transformed) {
						return payload, nil, NewOpenAIWSClientCloseError(coderws.StatusPolicyViolation, "invalid transformed websocket request payload", nil)
					}
					payload = transformed
				}
				if hooks != nil && hooks.MapRequestModel != nil {
			...
			if policyErr == nil && blocked == nil && isResponseCreate {
				if account.IsOpenAIOAuth() {
					ids := advanceCodexFingerprintTurn(c, account)
					if ids != nil {
						fingerprinted, changed, fingerprintErr := applyCodexFingerprintJSONBody(out, ids)
						if fingerprintErr != nil {
							return out, nil, fmt.Errorf("apply OpenAI Codex fingerprint metadata to websocket payload: %w", fingerprintErr)
						}
						if changed {
							out = fingerprinted
						}
		...
		Options: openaiwsv2.RelayOptions{
			...
			BeforeWriteClient: func(msgType coderws.MessageType, payload []byte, wroteDownstream bool) error {
				...
				if (eventType == "error" || eventType == "response.failed") && !failureAccountSideEffectsApplied && !isPreOutputRateLimit {
					failureAccountSideEffectsApplied = s.handleOpenAIWSFailureAccountSideEffects(ctx, account, usageMeta.failureModel(capturedSessionModel), handshakeHeaders, payload)
				}
				if eventType != "error" {
					return nil
				}
				...
				s.persistOpenAIWSRateLimitSignal(ctx, account, handshakeHeaders, payload, errCodeRaw, errTypeRaw, errMsgRaw, usageMeta.failureModel(capturedSessionModel))
				logOpenAIWSV2Passthrough(
