...
// It prefers an explicit instance hint when present, otherwise it resolves the
// original provider from the order or falls back to a registry provider.
func (s *PaymentService) GetWebhookProvider(ctx context.Context, providerKey, outTradeNo, instanceHint string) (payment.Provider, error) {
	if strings.TrimSpace(instanceHint) != "" {
		prov, err := s.getWebhookProviderByInstanceHint(ctx, providerKey, instanceHint)
		if err == nil {
			return prov, nil
		}
		slog.Warn("payment webhook instance hint provider resolution failed", "provider", providerKey, "instanceHint", instanceHint, "error", err)
	}
