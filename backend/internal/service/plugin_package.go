...
func (i *PluginPackageInstaller) Install(ctx context.Context, reader io.Reader, installedBy *int64) (*PluginInstallation, error) {
	...
	// Windows does not allow moving the uploaded package while zip.OpenReader
	// still holds the source file open. Close it explicitly before either
	// artifact is committed so install remains atomic on every platform.
