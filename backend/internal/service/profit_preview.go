...
// 分组利润控制离线预演：使用生产只读导出的分组配置、最终账号倍率、
...
const (
	// The account row is the single runtime source of truth. Its rate may be
	// maintained manually or synchronized by the centralized connection service.
	ProfitPreviewRateSourceAccount = "account_rate"
	ProfitPreviewWarningRateOne    = "account_rate_1_review"
)
...
func PreviewProfitAdmission(inputs []ProfitPreviewGroupInput, evalAt time.Time) []ProfitPreviewGroupReport {
	...
	for _, in := range inputs {
		...
		for _, account := range in.Accounts {
			...
			verdict := previewAccountProfitAdmission(account, effectiveGate, thresholdDefault, thresholdMinD)
...
func previewAccountProfitAdmission(
	...
	verdict := ProfitPreviewAccountVerdict{
		...
		RateSource: ProfitPreviewRateSourceAccount,
	}
	if account.RateMultiplier != nil && *account.RateMultiplier == 1 {
		verdict.Warnings = append(verdict.Warnings, ProfitPreviewWarningRateOne)
	}
