...
func TestPreviewProfitAdmissionUsesAccountRatesAndPreinitializesModels(t *testing.T) {
	...
	require.Equal(t, ProfitPreviewRateSourceAccount, byID[cheap.ID].RateSource)
	require.Empty(t, byID[cheap.ID].Warnings)
	...
	require.Contains(t, byID[expensive.ID].Warnings, ProfitPreviewWarningRateOne)
