...
func TestSettingService_GetAllSettings_UsesReferralDefaultsWhenUnset(t *testing.T) {
	repo := &settingGetAllRepoStub{
		values: map[string]string{},
	}
	svc := NewSettingService(repo, &config.Config{})
	...
	require.NoError(t, err)
	require.NotNil(t, settings)
	require.False(t, settings.ReferralEnabled)
	require.True(t, settings.ReferralLevel1Enabled)
	require.Equal(t, 0.0, settings.ReferralLevel1Rate)
	require.Equal(t, ReferralRewardModeFirstPaidOrder, settings.ReferralRewardMode)
	require.Equal(t, 7, settings.ReferralSettlementDelayDays)
	require.True(t, settings.ReferralBindBeforeFirstPaidOnly)
	require.True(t, settings.ReferralAllowManualInput)
	require.False(t, settings.ReferralWithdrawEnabled)
	require.Equal(t, 100.0, settings.ReferralWithdrawMinAmount)
	require.Equal(t, 5000.0, settings.ReferralWithdrawMaxAmount)
	require.Equal(t, 1, settings.ReferralWithdrawDailyLimit)
	require.Equal(t, 0.0, settings.ReferralWithdrawFeeRate)
	require.Equal(t, 0.0, settings.ReferralWithdrawFixedFee)
	require.True(t, settings.ReferralWithdrawManualReviewRequired)
	require.True(t, settings.ReferralRefundReverseEnabled)
	require.True(t, settings.ReferralNegativeCarryEnabled)
	require.Equal(t, ReferralSettlementCurrencyCNY, settings.ReferralSettlementCurrency)
	require.Equal(
		t,
		[]string{
