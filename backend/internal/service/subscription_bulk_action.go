...
func (input *BulkSubscriptionActionInput) Validate() error {
	...
	for _, id := range input.SubscriptionIDs {
		if _, synthetic := syntheticEntitlementIDFromSubscriptionID(id); id <= 0 && !synthetic {
			return infraerrors.BadRequest("INVALID_SUBSCRIPTION_ID", "subscription IDs must be positive legacy IDs or valid synthetic entitlement IDs")
		}
	}
	switch input.Action {
	case "extend":
...
func (s *SubscriptionService) BulkSubscriptionAction(ctx context.Context, input *BulkSubscriptionActionInput) (*BulkSubscriptionActionResult, error) {
	...
	for _, id := range input.SubscriptionIDs {
		...
		seen[id] = struct{}{}
		err := ctx.Err()
		if entitlementID, synthetic := syntheticEntitlementIDFromSubscriptionID(id); err == nil && synthetic {
			err = s.bulkEntitlementAction(ctx, entitlementID, input)
		} else if err == nil {
...
func (s *SubscriptionService) bulkEntitlementAction(ctx context.Context, entitlementID int64, input *BulkSubscriptionActionInput) error {
	switch input.Action {
	case "extend":
		_, err := s.adminAdjustEntitlement(ctx, entitlementID, input.Days)
		return err
	case "reset_quota":
		_, err := s.adminResetEntitlementQuota(ctx, entitlementID, input.Daily, input.Weekly, input.Monthly)
		return err
	case "revoke":
		return s.adminRevokeEntitlement(ctx, entitlementID)
	case "restore":
		_, err := s.adminRestoreEntitlement(ctx, entitlementID)
		return err
	default:
		return ErrInvalidInput
	}
