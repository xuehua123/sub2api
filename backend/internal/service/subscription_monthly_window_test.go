...
func TestLegacyWindowActivationAnchorsMonthlyWindowAtSubscriptionStart(t *testing.T) {
...
func TestThirtyDaySubscriptionDoesNotResetMonthlyQuotaBeforeExpiry(t *testing.T) {
	// Keep the term in the future so MonthlyResetTime's public wall-clock API
	// does not make this fixed-date test expire as the calendar advances.
	startsAt := time.Date(2099, 7, 1, 23, 30, 0, 0, time.UTC)
