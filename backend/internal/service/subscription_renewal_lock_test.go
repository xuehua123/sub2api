...
func TestAssignSubscriptionDoesNotReactivateRowSuspendedAfterStaleRead(t *testing.T) {
	...
	sub, outcome, err := svc.assignSubscriptionWithReuse(context.Background(), &AssignSubscriptionInput{
		UserID: 31, GroupID: 33, ValidityDays: 5, Notes: "renewed",
	}, false)

	require.NoError(t, err)
	require.Equal(t, subscriptionAssignmentReused, outcome)
	require.Equal(t, 1, repo.lockReads)
...
func TestAssignSubscriptionDoesNotExtendRowRenewedAfterStaleRead(t *testing.T) {
	now := time.Date(2026, 8, 2, 12, 0, 0, 0, time.UTC)
	current := UserSubscription{
		ID: 29, UserID: 35, GroupID: 37, StartsAt: now, ExpiresAt: now.AddDate(0, 0, 30),
		Status: SubscriptionStatusActive, Notes: "renewed by first assignment",
	}
	repo := &lockingRenewalRepo{
		stale:   UserSubscription{ID: 29, UserID: 35, GroupID: 37, ExpiresAt: now.Add(-time.Hour), Status: SubscriptionStatusExpired},
		current: current,
	}
	svc := NewSubscriptionService(&subscriptionGroupRepoStub{group: &Group{ID: 37, SubscriptionType: SubscriptionTypeSubscription}}, repo, nil, nil, nil)
	svc.now = func() time.Time { return now }

	sub, outcome, err := svc.assignSubscriptionWithReuse(context.Background(), &AssignSubscriptionInput{
		UserID: 35, GroupID: 37, ValidityDays: 30, Notes: "concurrent renewal",
	}, false)
...
func TestAssignSubscriptionV2DoesNotGrantAliasWhenLockedRowIsSuspended(t *testing.T) {
	now := time.Date(2026, 8, 2, 12, 0, 0, 0, time.UTC)
	current := UserSubscription{
		ID: 37, UserID: 41, GroupID: 1, StartsAt: now.AddDate(0, 0, -10), ExpiresAt: now.Add(-time.Hour),
		Status: SubscriptionStatusSuspended, Notes: "suspended",
	}
	repo := &lockingRenewalRepo{
		stale:   UserSubscription{ID: 37, UserID: 41, GroupID: 1, ExpiresAt: now.Add(-time.Hour), Status: SubscriptionStatusExpired},
		current: current,
	}
	...
	svc.userSubRepo = repo
	svc.now = func() time.Time { return now }

	sub, err := svc.AssignSubscription(context.Background(), &AssignSubscriptionInput{
		UserID: 41, GroupID: 1, PlanID: 77, ValidityDays: 5, Notes: "renewed",
	})
	...
	require.Empty(t, entRepo.entitlements)
}
