...
func TestBatchImageBalanceHoldNormalizeKeepsRawFingerprintAndQuantizesAmounts(t *testing.T) {
	const raw = 0.000078125
	cmd := &BatchImageBalanceHoldCommand{
		RequestID:     "batch-5229",
		UserID:        1,
		APIKeyID:      2,
		BatchID:       "batch-1",
	...
	require.Equal(t, QuantizeUsageBillingAmount(raw), cmd.HoldAmount)
	require.Equal(t, QuantizeUsageBillingAmount(raw), cmd.ActualAmount)
