...
func TestUserCanBindGroup(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			name:    "exclusive-only and public allowlist policies take their intersection",
			user:    User{RestrictToAllowedGroups: true, RestrictPublicGroups: true, AllowedGroups: []int64{publicGroupA}},
			groupID: publicGroupA,
			want:    false,
		},
	} {
