...
func ProvideOpsAlertEvaluatorService(
	...
	svc := NewOpsAlertEvaluatorService(opsService, opsRepo, emailService, accountTestService, redisClient, cfg, proxyRepo)
	svc.Start()
	return svc
}
...
func ProvideSettingService(settingRepo SettingRepository, groupRepo GroupRepository, proxyRepo ProxyRepository, cfg *config.Config) *SettingService {
	...
	connectivityCtx, connectivityCancel := context.WithTimeout(context.Background(), 10*time.Second)
	if err := svc.LoadConnectivityProbeSettings(connectivityCtx); err != nil {
		logger.LegacyPrintf("service.setting", "Warning: load connectivity probe settings failed: %v", err)
	}
	connectivityCancel()
	svc.StartConnectivityProbeRefresh(context.Background())
...
var ProviderSet = wire.NewSet(
	...
	wire.Bind(new(SubscriptionEntitlementsRuntimeProvider), new(*SettingService)),
...
// ProvidePluginManager binds host services through Wire so regeneration preserves
// the account directory capability before any plugin runtime starts.
