    ...
    >
      ...
      <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <div>
          ...
          <input v-model.number="form.rate_multiplier" type="number" min="0" step="0.001" class="input" :disabled="upstreamRateMultiplierSyncEnabled && (accountCategory === 'apikey' || (form.platform === 'antigravity' && antigravityAccountType === 'upstream'))" />
      ...
      <div v-if="accountCategory === 'apikey' || (form.platform === 'antigravity' && antigravityAccountType === 'upstream')" class="border-t border-gray-200 pt-4 dark:border-dark-600">
        <div class="flex items-start justify-between gap-4">
          ...
          <button
            type="button"
            data-testid="create-upstream-rate-sync-toggle"
        ...
        <div v-if="upstreamRateMultiplierSyncEnabled" class="mt-4 max-w-2xl">
          ...
          <div class="flex flex-wrap items-end justify-between gap-3">
            ...
            <button type="button" data-testid="create-upstream-rate-sync-discover" class="btn-secondary text-sm" :disabled="upstreamRateMultiplierSyncDiscovering" @click="discoverUpstreamRateMultiplierGroups">
          ...
          <div class="mt-4 grid gap-4 sm:grid-cols-2">
            ...
            <div v-if="upstreamManagementRequiresRemoteUserID">
              ...
              <input v-model.number="upstreamRateMultiplierSyncRemoteUserID" data-testid="create-upstream-rate-sync-remote-user-id" type="number" min="1" step="1" class="input" />
            </div>
          </div>
          ...
          <div v-else class="mt-4 grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <input v-model.trim="upstreamManagementUsername" data-testid="create-upstream-rate-sync-username" type="text" autocomplete="username" class="input" />
            </div>
            <div>
              <label class="input-label">{{ t('admin.accounts.upstreamRateSync.password') }}</label>
              <input v-model="upstreamManagementPassword" data-testid="create-upstream-rate-sync-password" type="password" autocomplete="new-password" class="input" />
            </div>
          </div>
        </div>
      </div>
...
const discoverUpstreamRateMultiplierGroups = async () => {
  ...
  const hasCredentials = upstreamRateMultiplierSyncAuthMode.value === 'access_token'
...
const buildAnthropicExtra = (base?: Record<string, unknown>): Record<string, unknown> | undefined => {
  ...
  if (anthropicCachedTokensInInput.value) {
    extra.anthropic_cached_tokens_in_input = true
  } else {
    delete extra.anthropic_cached_tokens_in_input
  }
...
const doCreateAccount = async (payload: CreateAccountRequest) => {
  const supportsUpstreamRateSync = accountCategory.value === 'apikey' ||
  ...
  if (supportsUpstreamRateSync && upstreamRateMultiplierSyncEnabled.value) {
    ...
    const hasCredentials = upstreamRateMultiplierSyncAuthMode.value === 'access_token'
      ? Boolean(upstreamManagementAccessToken.value.trim())
      : Boolean(upstreamManagementUsername.value.trim() && upstreamManagementPassword.value)
    if (!hasRemoteUserID || !hasCredentials) {
      appStore.showError(t('admin.accounts.upstreamRateSync.credentialsRequired'))
      return
    }
  }
  ...
  if (supportsUpstreamRateSync && upstreamRateMultiplierSyncEnabled.value) {
    ...
    extra.upstream_rate_multiplier_sync_auth_mode = upstreamRateMultiplierSyncAuthMode.value
