...
describe('BulkEditAccountModal', () => {
  ...
  it('批量编辑可开启账号级上游请求压缩', async () => {
    const wrapper = mountModal({
      selectedPlatforms: ['openai'],
      selectedTypes: ['oauth']
    })

    await wrapper.get('#bulk-edit-upstream-gzip-enabled').setValue(true)
    await wrapper.get('#bulk-edit-upstream-gzip-toggle').trigger('click')
    await wrapper.get('#bulk-edit-account-form').trigger('submit.prevent')
    await flushPromises()
  ...
  it('Codex 指纹模式仅对 OpenAI OAuth 展示和提交', async () => {
    ...
    await oauthWrapper.get('#bulk-edit-openai-codex-fingerprint-mode-enabled').setValue(true)
    ...
    expect(setupTokenWrapper.find('#bulk-edit-openai-codex-fingerprint-mode-enabled').exists()).toBe(false)
  })
  ...
  it('筛选批量编辑仅在精确 OpenAI OAuth 过滤时展示并提交 Codex 指纹模式', async () => {
    ...
    expect(unsafePreviewWrapper.find('#bulk-edit-openai-codex-fingerprint-mode-enabled').exists()).toBe(false)
    ...
    await exactOAuthFilterWrapper.get('#bulk-edit-openai-codex-fingerprint-mode-enabled').setValue(true)
