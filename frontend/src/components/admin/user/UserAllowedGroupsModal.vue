      ...
      <div v-else class="space-y-6">
        ...
        <div v-if="publicGroups.length > 0">
          <div class="mb-3 flex flex-wrap items-center gap-2">
            ...
            <h4 class="text-sm font-semibold text-gray-700 dark:text-gray-300">{{ restrictPublicGroups ? t('admin.users.publicGroupsRestricted') : t('admin.users.publicGroups') }}</h4>
            <span class="text-xs text-gray-400">({{ publicGroupConfigs.length }})</span>
            <span v-if="user.restrict_to_allowed_groups" class="rounded-full bg-amber-100 px-2 py-0.5 text-xs font-medium text-amber-700 dark:bg-amber-900/30 dark:text-amber-300">
              {{ t('admin.users.publicGroupsExclusiveOnly') }}
            </span>
          ...
          <div class="grid gap-3">
            ...
            >
              <div class="flex items-center gap-4">
                <!-- 未开启公开分组限制时公开分组恒可用；开启后才是真实白名单开关 -->
                <div class="flex-shrink-0">
                  <input
...
const load = async () => {
  ...
  try {
    ...
    // 允许管理所有活跃分组的用户级白名单。订阅分组仍会在后端校验订阅权益，
    // 但开启公开分组限制时，它们也必须能在这里被加入白名单。
    ...
    groupConfigs.value = groups.value.map((g) => ({
      ...
      // 专属分组始终检查 allowed_groups；公开分组只在开启公开白名单时检查。
...
const handleSave = async () => {
  ...
  try {
    // 专属分组中被勾选的始终保存；开启公开白名单时同时保存勾选的公开分组。
