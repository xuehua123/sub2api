                ...
                <template v-if="tokenIntervals(m).length">
                  <div v-for="(iv, idx) in tokenIntervals(m)" :key="idx" class="leading-5">
					...
					<span v-if="iv.cache_write_5m_price != null">
					  {{ t('modelPlaza.table.cacheWrite') }} 5m {{ paidPerMillion(iv.cache_write_5m_price, period) }}
					</span>
					<span v-if="iv.cache_write_1h_price != null">
					  / {{ t('modelPlaza.table.cacheWrite') }} 1h {{ paidPerMillion(iv.cache_write_1h_price, period) }}
					</span>
					<span v-if="iv.cache_write_5m_price == null && iv.cache_write_1h_price == null && iv.cache_write_price != null">
					  {{ t('modelPlaza.table.cacheWrite') }} {{ paidPerMillion(iv.cache_write_price, period) }}
                    </span>
                    <span v-if="iv.cache_write_price != null && iv.cache_read_price != null"> / </span>
                    <span v-if="iv.cache_read_price != null">
                      {{ t('modelPlaza.table.cacheRead') }} {{ paidPerMillion(iv.cache_read_price, period) }}
                    </span>
                ...
                <template v-else>
                  <div>
					...
					<template v-if="m.pricing?.cache_write_5m_price != null || m.pricing?.cache_write_1h_price != null">
					  5m {{ paidPerMillion(m.pricing?.cache_write_5m_price, period) }} / 1h {{ paidPerMillion(m.pricing?.cache_write_1h_price, period) }}
					</template>
					<template v-else>{{ paidPerMillion(m.pricing?.cache_write_price, period) }}</template>
                  </div>
                  <div>
