...
describe('PlazaModelPricingTable', () => {
  ...
  it('token 阶梯缓存价格按档展示并保留显式零', () => {
    ...
    expect(text).toContain('modelPlaza.table.cacheWriteShort $0.00')
    expect(text).toContain('modelPlaza.table.cacheReadShort $0.20')
  })
...
describe('PlazaModelPricingTable 长上下文阶梯', () => {
  ...
  it('无标签的多档展示完整有效区间,并按下限升序展示', () => {
    ...
    expect(rows.map((r) => r.text().split(/\s+/)[0])).toEqual(['≤100K', '100K–200K', '200K–1M', '>1M'])
  })
