export default {
    accounts: {
      ...
      autoRateMultiplierPriorityHint: 'When enabled, priorities are automatically ordered from lower to higher account rates; equal rates share the same priority.',
      autoRateMultiplierPriorityUpdated: 'Automatic rate-based priority sorting updated',
      autoRateMultiplierPriorityUpdateFailed: 'Failed to update automatic rate-based priority sorting',
      rateMultiplierPriorityConfigTitle: 'Rate Priority Settings',
      rateMultiplierPriorityConfigDescription: 'Reorders both account and group-bound priorities from lower to higher rates for enabled, schedulable accounts. Equal rates share the same priority. Saving changes the next scheduled run only.',
      rateMultiplierPriorityConfigUpdated: 'Rate priority settings updated',
      rateMultiplierPriorityConfigUpdateFailed: 'Failed to update rate priority settings',
      rateMultiplierPriorityInterval: 'Recheck interval (minutes)',
      rateMultiplierPriorityIntervalHint: '1 to 60 minutes. The scheduler keeps serving requests while rules are refreshed.',
      rateMultiplierPriorityStep: 'Priority step',
      rateMultiplierPriorityStepHint: 'For example, a step of 10 produces priorities 1, 11, 21 for successive distinct rates.',
      rateMultiplierPriorityInvalidInterval: 'The recheck interval must be a whole number from 1 to 60.',
      rateMultiplierPriorityInvalidStep: 'The priority step must be a whole number from 1 to 1000.',
      ...
      upstreamRateSync: {
        title: 'Sync Upstream Group Rate',
        description: 'Uses an upstream user-management identity to sync this account\'s group rate every 5 minutes. The same encrypted credential also reads the upstream account balance. Scheduling priority changes only when global automatic sorting is enabled.',
        group: 'Detected Upstream Group',
        groupPlaceholder: 'e.g. plus',
        groupHint: 'For Sub2API, the account API key is matched to the management key list and group rate table. Manual selection is only a fallback for upstreams that cannot expose this mapping.',
        groupRequired: 'Detect this account\'s upstream group before enabling rate sync',
        groupSelectPlaceholder: 'Detect upstream groups first',
        discover: 'Detect upstream groups',
        discovering: 'Detecting...',
        discoverSuccess: 'Upstream group detected',
        discoverFailed: 'Unable to detect the upstream or load its groups',
        notDetected: 'Not detected yet',
        managedHint: 'Managed by the upstream group rate and cannot be edited manually',
        provider: 'Detected Upstream Type',
        providerNewAPI: 'NewAPI',
        providerRixAPI: 'RixAPI',
        ...
        credentialsConfigured: 'Management credentials configured'
      },
      expiresAt: 'Expires At',
      ...
      upstreamGzipDesc: 'Controls whether this account may gzip request bodies when forwarding upstream. OpenAI OAuth is off by default; internal codex2api relay accounts can opt in.',
      upstreamGzipOn: 'On',
      upstreamGzipOff: 'Off',
