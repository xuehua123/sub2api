export default {
    settings: {
      ...
      site: {
        ...
        connectivity: {
          title: 'Client Network Connectivity Test',
          description: 'Let users test how their current device and network connect to each API URL from the API Keys page',
          enabled: 'Enable connectivity testing',
          clientIpEnabled: 'Show client egress IP',
          clientIpHint: 'Only takes effect when the trusted proxy chain and denied node CIDRs are configured correctly; otherwise the backend always returns an empty value',
          geoipStatus: 'GeoIP database',
          geoipStatusReady: 'Ready — coarse region lookup is available',
          geoipStatusNotConfigured: 'Not configured — region display is off',
          geoipStatusUnavailable: 'Unavailable — the configured database could not be loaded',
          allowedOrigins: 'Allowed API Origins',
          allowedOriginsHint: 'Origins can only be selected from the current API endpoint URLs; URLs on the same origin are tested once',
          selectedOriginCount: '{count} enabled',
          defaultEndpoint: 'Default API endpoint',
          configuredEndpoint: 'Custom API endpoint',
          unmatchedOrigin: 'Not in current endpoints',
          noOrigins: 'Configure a valid API endpoint URL first',
          requestBudget: '{origins} origins, up to {requests} probe requests per run (limit 250)',
          advanced: 'Advanced settings',
          samplingTitle: 'Sampling',
          samples: 'Measured samples',
          warmup: 'Warm-up requests',
          concurrency: 'Concurrent origins',
          timeoutMs: 'Request timeout (ms)',
          rateLimitTitle: 'Server probe rate limit',
          ipRpm: 'Requests per IP per minute',
          burst: 'Burst requests per IP',
          thresholdsTitle: 'Grade thresholds',
          thresholdsHint: 'Excellent must be stricter than Good. Changing a threshold automatically updates the grading version and invalidates old cached results',
          minimumSuccessRate: 'Minimum success rate',
          consecutiveTimeouts: 'Consecutive timeout limit',
          excellent: 'Excellent',
          good: 'Good',
          successRate: 'Minimum success rate',
          maxP95: 'Maximum P95 (ms)',
          maxMad: 'Maximum jitter (ms)',
          errors: {
            allowedOriginInvalid: 'An allowed origin is invalid. Remove the invalid entry and try again',
            allowedOriginRequired: 'Select at least one currently valid API origin before enabling connectivity testing',
            samplesRange: 'Measured samples must be an integer between 5 and 20',
            warmupRange: 'Warm-up requests must be an integer between 0 and 2',
            concurrencyRange: 'Concurrent origins must be an integer between 1 and 3',
            timeoutRange: 'Request timeout must be an integer between 2000 and 15000 milliseconds',
            ipRpmRange: 'Requests per IP per minute must be an integer between 1 and 10000',
            burstRange: 'Burst requests per IP must be an integer between 1 and 1000',
            gradingVersionInvalid: 'The grading version is invalid. Reload settings and try again',
            successRateOrder: 'Success-rate thresholds must be ordered: Excellent above Good, and Good above the minimum',
            p95Order: 'P95 thresholds must be positive, and the Good limit must exceed the Excellent limit',
            madOrder: 'Jitter thresholds cannot be negative, and the Good limit must exceed the Excellent limit',
            consecutiveTimeoutsRange: 'The consecutive timeout limit must be an integer between 1 and 20',
            endpointLimitExceeded: 'Connectivity testing can expose at most 11 API URLs. Reduce endpoints under the selected origins',
            requestBudgetExceeded: 'The connectivity test exceeds the 250-request budget. Reduce endpoints, samples, or warm-up requests',
            burstBelowRequestBudget: 'The burst limit must cover one full test run, otherwise normal probes will be rate-limited',
          },
        },
      ...
      referral: {
        creditConversionRateDesc: "10 converts 1 commission into 10 balance; 0.8 converts 100 commission into 80 balance.",
        creditConversionRate: "Balance conversion multiplier",
      ...
      lobehub: {
        runtimeConfigVersionPlaceholder: "Enter a configuration version",
        oidcIssuerPlaceholder: "https://api.example.com/lobehub/oidc",
        hideImportButtonHint: "Hide the LobeHub import button in the user interface",
        defaultModelPlaceholder: "Enter the default model name",
        clientSecretPlaceholder: "Enter OIDC client secret",
        clientSecretConfiguredPlaceholder: "Configured; leave blank to keep current secret",
        clientSecret: "OIDC client secret",
        clientIdPlaceholder: "Enter OIDC client ID",
