export default {
  ...
  auth: {
    ...
    lobehub: {
      title: '正在连接 LobeHub',
      continuing: '正在为你打开 LobeHub，请稍候...',
      invalidRequest: 'LobeHub 登录请求无效，请返回 LobeHub 重试。',
      continueFailed: '无法继续打开 LobeHub，请稍后重试。',
      selectKeyTitle: '选择聊天 API 密钥',
      selectKeyDescription: '请选择用于 LobeHub 聊天的 API 密钥。',
      noKeys: '当前没有可用的 API 密钥，请先创建一个。',
      keysLoadFailed: '加载 API 密钥失败',
      keyRequired: '请选择一个 API 密钥',
      continueWithKey: '使用此密钥继续'
    },
    linuxdo: {
  ...
  admin: {
    ...
    settings: {
      ...
      lobehub: {
        ...
        enabledHint: '为用户显示 LobeHub 打开入口并启用 OIDC 服务。',
        chatUrl: 'LobeHub 聊天 URL',
        chatUrlPlaceholder: 'https://chat.example.com',
        chatUrlHint: '用户实际访问的 LobeHub 聊天地址。',
        oidcIssuer: 'OIDC 签发者 URL',
        oidcIssuerPlaceholder: 'https://api.example.com',
        oidcIssuerHint: 'LobeHub 通用 OIDC 配置中使用的 issuer。',
        clientId: 'OIDC 客户端 ID',
        clientIdPlaceholder: 'sub2api-lobehub',
        clientIdHint: '必须与 LobeHub 环境配置中的客户端 ID 一致。',
        clientSecret: 'OIDC 客户端密钥',
        clientSecretPlaceholder: '输入 OIDC 客户端密钥',
        clientSecretConfiguredPlaceholder: '已配置，留空则保持不变',
        clientSecretHint: 'Token 端点校验客户端时使用。',
        clientSecretConfiguredHint: '密钥已配置，留空以保留当前值，填写则轮换密钥。',
        defaultProvider: '默认服务商',
        defaultProviderHint: '导入到 LobeHub 设置时使用的服务商名称。',
        defaultModel: '默认模型',
        defaultModelPlaceholder: 'gpt-5.4-mini',
        defaultModelHint: '可选，传给 LobeHub 设置的默认模型。',
        runtimeConfigVersion: '运行时配置版本',
        runtimeConfigVersionPlaceholder: '2026-04-07',
        runtimeConfigVersionHint: '导入行为变化后递增此值，让 LobeHub 刷新启动配置。',
        hideImportButton: '隐藏 LobeHub 打开按钮',
        hideImportButtonHint: '开启后用户 API 密钥列表不显示 LobeHub 打开入口。'
      },
