export default {
    accounts: {
      ...
      upstreamRateSync: {
        groupSelectPlaceholder: '请先检测并加载上游分组',
        discover: '检测上游分组',
        discovering: '检测中...',
        discoverSuccess: '已识别上游归属分组',
        discoverFailed: '无法识别上游或加载分组',
        notDetected: '尚未识别',
        provider: '上游类型',
        ...
        credentialsConfigured: '管理端凭据已配置',
        title: '同步上游分组倍率',
        description: '使用上游用户管理端身份每 5 分钟读取并更新账号倍率；同一份加密凭据也用于读取上游真实余额。调度优先级仅在全局自动排序开启时更新。',
        group: '上游归属分组',
        groupPlaceholder: '例如 plus',
        groupHint: 'Sub2API 会通过账号 Key 与上游管理端 Key 列表、分组倍率表精确匹配；只有不支持该映射的上游才需要手动兜底。不会读取或修改本地分组绑定。',
        groupRequired: '请先检测此账号的上游归属分组后再启用倍率同步',
        managedHint: '由上游分组倍率自动管理，不能手动修改'
      },
      expiresAt: '过期时间',
