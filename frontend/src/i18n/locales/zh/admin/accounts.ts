export default {
    accounts: {
      ...
      autoRateMultiplierPriorityHint: '启用后按账号倍率从低到高自动重排优先级；相同倍率保持相同优先级。',
      autoRateMultiplierPriorityUpdated: '已更新自动按倍率排序设置',
      autoRateMultiplierPriorityUpdateFailed: '自动按倍率排序设置更新失败',
      rateMultiplierPriorityConfigTitle: '倍率排序设置',
      rateMultiplierPriorityConfigDescription: '只按启用且可调度账号的倍率从低到高重排账号优先级和分组内优先级；相同倍率保持同一优先级。保存只影响下一次计划执行，不会立即重排。',
      rateMultiplierPriorityConfigUpdated: '倍率排序设置已更新',
      rateMultiplierPriorityConfigUpdateFailed: '倍率排序设置更新失败',
      rateMultiplierPriorityInterval: '检查间隔（分钟）',
      rateMultiplierPriorityIntervalHint: '可设 1 到 60 分钟。规则刷新期间不会中断正在处理的请求。',
      rateMultiplierPriorityStep: '优先级步长',
      rateMultiplierPriorityStepHint: '例如步长为 10 时，连续的不同倍率会得到 1、11、21。',
      rateMultiplierPriorityInvalidInterval: '检查间隔必须是 1 到 60 的整数。',
      rateMultiplierPriorityInvalidStep: '优先级步长必须是 1 到 1000 的整数。',
      ...
      upstreamConnectionBalance: {
        ...
        refreshFailed: '上游余额刷新失败'
      },
      upstreamMultiplierSync: {
        loading: '读取中',
        loadingTitle: '正在读取上游倍率观测状态',
        loadFailed: '读取失败',
        loadFailedTitle: '无法读取上游连接数据，因此无法验证账号倍率。',
        unbound: '未绑定',
        unboundTitle: '该账号未绑定共享上游连接，无法验证账号倍率。',
        pending: '待同步',
        pendingTitle: '该账号已绑定上游连接，但倍率观测尚未完成。上游连接：{connection}',
        unresolved: '待解析',
        unresolvedTitle: '已同步上游连接数据，但未能确定此 API Key 所在分组。上游连接：{connection}',
        failed: '同步失败',
        failedTitle: '上游倍率同步失败。上游连接：{connection}；错误：{error}',
        multiplierUnavailable: '未提供倍率',
        ...
        mismatch: '有差异',
        mismatchTitle: '账号倍率与已观测的上游分组倍率不一致。上游连接：{connection}；上游分组：{group}；上游倍率：{upstream}x；账号倍率：{account}x；同步时间：{observedAt}。此标识不会自动修改账号计费倍率。',
        synchronized: '已同步',
        synchronizedTitle: '账号倍率已与上游分组倍率核对一致。上游连接：{connection}；上游分组：{group}；上游倍率：{upstream}x；账号倍率：{account}x；同步时间：{observedAt}。此标识只验证，不会自动修改账号计费倍率。'
      },
      ...
      upstreamGzip: '上游请求压缩',
      upstreamGzipDesc: '控制该账号转发到上游时是否允许 gzip 请求体。OpenAI OAuth 默认关闭；内部 codex2api 中转账号可按需开启。',
      upstreamGzipOn: '开启',
      upstreamGzipOff: '关闭',
