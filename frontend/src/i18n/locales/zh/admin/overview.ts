export default {
    ...
    backup: {
      ...
      actions: {
        download: '下载',
        downloadParts: '下载分卷',
        downloadPartsHint: '请按顺序下载全部分卷后拼接 gzip 字节流：Linux/macOS 使用 cat payload.part-* > backup.sql.gz；Windows 使用 {windowsCommand}。',
        partLabel: '第 {index} 卷',
    ...
    users: {
      ...
      restrictPublicGroupsHint: '开启后，该用户仅能使用下方勾选的公开分组；若同时开启“仅专属分组”，公开分组仍不可用。',
      publicGroupsRestricted: '公开分组（已限制）',
      publicGroupsExclusiveOnly: '“仅专属分组”模式下不可用',
      defaultRate: '默认倍率',
      customRate: '专属倍率',
      useDefaultRate: '使用默认',
