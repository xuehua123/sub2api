export default {
    settings: {
      ...
      site: {
        ...
        connectivity: {
          title: '用户本机连接检测',
          description: '让用户在 API 密钥页面检测当前设备和网络访问各 API URL 的连接表现',
          enabled: '启用连接检测',
          clientIpEnabled: '展示用户出口 IP',
          clientIpHint: '仅在可信代理链和禁止返回的节点 CIDR 均已正确配置时生效；不满足安全条件时后端始终返回空',
          geoipStatus: 'GeoIP 数据库',
          geoipStatusReady: '已就绪 — 可提供粗粒度地区归属',
          geoipStatusNotConfigured: '未配置 — 地区展示已关闭',
          geoipStatusUnavailable: '数据不可用 — 配置的数据库无法加载',
          allowedOrigins: '允许检测的 API Origin',
          allowedOriginsHint: '只能从当前 API 端点地址中选择；同一 Origin 下的多个 URL 只检测一次',
          selectedOriginCount: '已启用 {count} 个',
          defaultEndpoint: '默认 API 端点',
          configuredEndpoint: '自定义 API 端点',
          unmatchedOrigin: '不属于当前端点',
          noOrigins: '请先配置有效的 API 端点地址',
          requestBudget: '{origins} 个 Origin，预计每轮最多 {requests} 个探针请求（上限 250）',
          advanced: '高级设置',
          samplingTitle: '采样参数',
          samples: '正式样本数',
          warmup: '预热次数',
          concurrency: '最大并发 Origin',
          timeoutMs: '单次超时（毫秒）',
          rateLimitTitle: '服务端探针限流',
          ipRpm: '单 IP 每分钟请求数',
          burst: '单 IP 突发请求数',
          thresholdsTitle: '评分阈值',
          thresholdsHint: '优秀必须严格高于良好；阈值变化后评分版本会自动更新，使旧缓存失效',
          minimumSuccessRate: '最低成功率',
          consecutiveTimeouts: '连续超时上限',
          excellent: '优秀',
          good: '良好',
          successRate: '最低成功率',
          maxP95: 'P95 上限（ms）',
          maxMad: '抖动上限（ms）',
          errors: {
            allowedOriginInvalid: '允许检测的 Origin 配置无效，请移除异常项后重试',
            allowedOriginRequired: '启用连接检测前，至少选择一个当前有效的 API Origin',
            samplesRange: '正式样本数必须是 5-20 之间的整数',
            warmupRange: '预热次数必须是 0-2 之间的整数',
            concurrencyRange: '最大并发 Origin 必须是 1-3 之间的整数',
            timeoutRange: '单次超时必须是 2000-15000 毫秒之间的整数',
            ipRpmRange: '单 IP 每分钟请求数必须是 1-10000 之间的整数',
            burstRange: '单 IP 突发请求数必须是 1-1000 之间的整数',
            gradingVersionInvalid: '评分版本无效，请重新加载设置后再试',
            successRateOrder: '成功率阈值必须满足：优秀高于良好，良好高于最低成功率',
            p95Order: 'P95 阈值必须为正数，且良好上限必须高于优秀上限',
            madOrder: '抖动阈值不能为负数，且良好上限必须高于优秀上限',
            consecutiveTimeoutsRange: '连续超时上限必须是 1-20 之间的整数',
            endpointLimitExceeded: '连接检测最多可公开 11 个 API URL，请减少已选 Origin 下的端点数量',
            requestBudgetExceeded: '单轮连接检测请求预算超过 250，请减少端点、样本数或预热次数',
            burstBelowRequestBudget: '突发请求上限必须不低于本轮预计请求数，否则正常检测会被限流',
          },
        },
        contactInfo: '客服联系方式',
      ...
      referral: {
        creditConversionRateDesc: "10 表示返佣 1 元转入 10 额度，0.8 表示返佣 100 元转入 80 额度。",
      ...
      lobehub: {
        runtimeConfigVersionPlaceholder: "输入配置版本",
        oidcIssuerPlaceholder: "https://api.example.com/lobehub/oidc",
        hideImportButtonHint: "隐藏用户界面的 LobeHub 导入按钮",
        defaultModelPlaceholder: "输入默认模型名称",
        clientSecretPlaceholder: "输入 OIDC 客户端密钥",
        clientSecretConfiguredPlaceholder: "已配置，留空保留当前密钥",
        clientSecret: "OIDC 客户端密钥",
        clientIdPlaceholder: "输入 OIDC 客户端 ID",
