export default {
  ...
  keys: {
    ...
    connectivity: {
      ...
      start: '开始检测',
      retry: '重新检测',
      loadingConfig: '正在加载检测配置...',
      completeAnnouncement: '连接检测已完成。',
      testing: '检测中',
      notTested: '未检测',
      incomplete: '未完成',
      rateLimited: '过于频繁',
      recommended: '推荐',
      testedAt: '检测于 {time}',
      disclaimer: '检测结果仅代表测试时，您当前设备和当前网络访问各 API URL 的连接表现。结果可能受运营商、地区、Wi-Fi、代理/VPN 及测试时间影响，不代表该端点对其他用户或其他网络环境的表现。',
      configUnavailable: '暂时无法加载有效的检测配置，请稍后重试。',
      rateLimitedMessage: '检测过于频繁，请稍后重试。',
      incompleteMessage: '本次检测未完成，请保持页面可见并确认当前网络可用后重试。',
      exitIP: '本次网络出口：',
      splitExit: '不同 API URL 使用了不同网络出口，可能存在代理、VPN 或分流规则。',
      unknownExit: '当前网络出口无法识别',
      exitIPHint: '出口 IP 仅表示该 API URL 在本次检测中看到的您的公网出口。若与日常网络不符，可能正在使用代理、VPN 或分流规则。',
      networkSection: '当前设备网络',
      publicEgress: '公网出口：',
      ipLocation: 'IP 归属地：',
      estimated: '（估算）',
      ...
      messages: {
        excellent: '您当前网络访问此 API URL 表现优秀，推荐使用。',
        good: '您当前网络访问此 API URL 表现良好，可以使用。',
        fair: '您当前网络可以访问此 API URL，体验可能存在波动。',
        not_recommended: '在您当前网络环境下，建议选择其他 API URL。',
      },
    },
    allGroups: '全部分组',
    allStatus: '全部状态',
