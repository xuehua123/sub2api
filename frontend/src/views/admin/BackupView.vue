        ...
        >
          ...
          <div class="relative max-h-[85vh] w-full max-w-lg overflow-y-auto rounded-xl bg-white p-6 shadow-2xl dark:bg-dark-800">
            ...
            <p class="mb-4 text-sm text-gray-500 dark:text-gray-400">
              {{ t('admin.backup.actions.downloadPartsHint', { windowsCommand: windowsPartsCommand }) }}
            </p>
            <div class="space-y-2">
              <div
...
const windowsPartsCommand = computed(() => {
  const partNames = [...downloadParts.value]
    .sort((a, b) => a.index - b.index)
    .map(part => `payload.part-${String(part.index).padStart(6, '0')}`)
  return partNames.length > 0
    ? `copy /b ${partNames.join('+')} backup.sql.gz`
    : 'copy /b payload.part-* backup.sql.gz'
})
