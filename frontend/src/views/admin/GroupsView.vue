      ...
      >
        ...
        <div class="border-t border-gray-200 pt-4 mt-4 dark:border-dark-400">
          <div class="flex flex-wrap items-start justify-between gap-3">
            ...
            <button type="button" class="btn btn-secondary shrink-0 whitespace-nowrap" @click="addEditGroupPricing">
              <Icon name="plus" size="sm" class="mr-1" />{{ t("admin.groups.modelPricing.add") }}
            </button>
          </div>
          <label class="mt-3 flex items-start gap-2">
...
const groupPricingToAPI = (
  ...
  pricing
    ...
    .map((entry) => {
      const result: ChannelModelPricing = {
        platform,
        models: entry.models,
      ...
      const cacheWrite5m = mTokToPerToken(entry.cache_write_5m_price);
      const cacheWrite1h = mTokToPerToken(entry.cache_write_1h_price);
      if (cacheWrite5m != null) result.cache_write_5m_price = cacheWrite5m;
      if (cacheWrite1h != null) result.cache_write_1h_price = cacheWrite1h;
      return result;
    });
...
const allColumns = computed<Column[]>(() => {
  ...
  return [
    ...basic.slice(0, 3),
    { key: "billing_type", label: t("admin.groups.columns.accessSources"), sortable: true },
...
const handleUpdateGroup = async () => {
  ...
  if (editForm.platform === "openai" && editCodexManifestRef.value && !editCodexManifestRef.value.validate()) {
    return;
  }
...
// subscription_type is derived from access capabilities. Visibility remains an
// independent choice so public subscription groups can opt into plan auto grant.
watch(
