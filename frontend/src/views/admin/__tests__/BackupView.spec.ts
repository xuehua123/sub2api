...
vi.mock('vue-i18n', () => ({
  useI18n: () => ({
    t: (key: string, params?: Record<string, unknown>) => {
      if (params?.windowsCommand !== undefined) return `${key}:${params.windowsCommand}`
      return params?.index === undefined ? key : `${key}:${params.index}`
    },
  }),
}))
...
describe('admin BackupView 分卷备份', () => {
  ...
  it('显示分卷数并在下载时列出每个分卷链接', async () => {
    ...
    expect(document.body.textContent).toContain(
      'copy /b payload.part-000001+payload.part-000002+payload.part-000003 backup.sql.gz',
    )
  })
