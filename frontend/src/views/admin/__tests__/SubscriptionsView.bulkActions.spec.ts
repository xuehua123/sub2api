...
vi.mock('@/api/admin', () => ({
  adminAPI: {
    payment: { getPlans: vi.fn().mockResolvedValue({ data: [{ id: 7, name: 'Test plan', group_id: 7, group_ids: [7], for_sale: true, validity_days: 30, validity_unit: 'day' }] }) },
...
describe('subscription bulk operations', () => {
  ...
  it('assigns multiple users once and retries only failed users', async () => {
    ...
    form.getComponent({ name: 'Select' }).vm.$emit('change', 7)
    ...
    expect(bulkAssign).toHaveBeenCalledWith({ user_ids: [11, 22], group_id: 7, plan_id: 7, validity_days: 30 })
    ...
    await form.trigger('submit')
    await flushPromises()
    expect(bulkAssign).toHaveBeenLastCalledWith({ user_ids: [22], group_id: 7, plan_id: 7, validity_days: 30 })
