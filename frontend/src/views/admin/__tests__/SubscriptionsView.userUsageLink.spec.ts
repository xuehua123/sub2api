...
vi.mock('@/api/admin', () => ({
  adminAPI: {
    payment: { getPlans: vi.fn().mockResolvedValue({ data: [{ id: 3, name: 'Test plan', group_id: 3, group_ids: [3], for_sale: true, validity_days: 30, validity_unit: 'day' }] }) },
...
describe('admin subscription users', () => {
  ...
  it.each(['another', ''])('clears the assignment user immediately when input changes to %j', async (keyword) => {
    ...
    try {
      ...
      form.getComponent({ name: 'Select' }).vm.$emit('change', 3)
      const search = wrapper.get('[data-assign-user-search] input')
      await search.trigger('focus')
      ...
      expect(assignSubscription).toHaveBeenCalledWith({
        user_id: 84, group_id: 3, plan_id: 3, validity_days: 30
      })
    } finally {
      wrapper.unmount()
      vi.useRealTimers()
