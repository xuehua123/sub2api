...
const createProviderOptions = computed(() => KEY_GROUP_PROVIDERS.map((value) => ({
  ...
  count: accessGroupOptions.value.filter((group) => !group.disabled && getKeyGroupProvider(group.platform) === value).length
})))

const formGroupOptions = computed(() => showEditModal.value
  ? accessGroupOptions.value
  : accessGroupOptions.value.filter((group) => getKeyGroupProvider(group.platform) === createProvider.value)
)
