...
vi.mock('@/composables/useCurrencyResolver', () => ({
  useCurrencyResolver: () => ({
    convertUsdToCny: (value: number) => value * 7,
    convertUsdToCnyForLog: (value: number) => value * 7,
    formatCny: (value: number) => `¥${value.toFixed(2)}`
  })
}))
...
vi.mock('vue-router', () => ({ useRoute: () => ({ path: '/keys', query: {} }), useRouter: () => ({ push: vi.fn(), replace: vi.fn() }) }))
...
describe('user KeysView column settings', () => {
  beforeEach(() => {
    ...
    listKeys.mockReset()
    updateKey.mockReset()
    vi.mocked(keysAPI.createWithPayload).mockReset()
  ...
  describe('create provider selection', () => {
    ...
    it('clears the previous group on provider change and submits only the newly selected group', async () => {
      ...
      await wrapper.get('#key-form').trigger('submit')
      expect(keysAPI.createWithPayload).not.toHaveBeenCalled()
      expect(showError).toHaveBeenCalledWith('keys.groupRequired')

      await groupSelect(wrapper).vm.$emit('update:modelValue', 5)
      vi.mocked(keysAPI.createWithPayload).mockResolvedValue({ ...createApiKey(), group_id: 5 })
      await wrapper.get('#key-form').trigger('submit')
      await flushPromises()
      expect(keysAPI.createWithPayload).toHaveBeenCalledOnce()
      expect(keysAPI.createWithPayload).toHaveBeenCalledWith(expect.objectContaining({ name: 'My key', group_id: 5, access_source: 'balance' }))
    })
