// Retained tool guides, separated from navigation and rendering.
const guides = {
  cherry: {
    title: "Cherry Studio",
    body: () =>
      `<ol><li>打开“设置” → “模型服务”，新增 OpenAI 服务商。</li><li>API 地址填写 <code>${state.baseURL}/v1</code>。</li><li>粘贴您在皮皮虾 AI 控制台创建的 API Key。</li><li>点击获取模型或手动添加控制台可见的模型名称。</li></ol>${snippet("Base URL", `${state.baseURL}/v1`)}`,
  },
  cursor: {
    title: "Cursor",
    body: () =>
      `<ol><li>打开 Cursor Settings，进入 Models。</li><li>启用 OpenAI Compatible 或自定义 OpenAI API Provider。</li><li>Base URL 填写 <code>${state.baseURL}/v1</code>，再保存 API Key。</li><li>模型名以皮皮虾 AI 控制台的模型列表为准。</li></ol>${snippet("Base URL", `${state.baseURL}/v1`)}`,
  },
  claude: {
    title: "Claude Code",
  },
  openwebui: {
    title: "OpenWebUI",
    body: () =>
      `<ol><li>以管理员身份打开“Settings” → “Connections”。</li><li>添加 OpenAI API Connection。</li><li>URL 填写 <code>${state.baseURL}/v1</code>，API Key 填写皮皮虾 AI 密钥。</li><li>保存后刷新模型列表。</li></ol>${snippet("Connection URL", `${state.baseURL}/v1`)}`,
  },
  python: {
    title: "Python SDK",
  },
  chatbox: {
    title: "Chatbox",
    body: () =>
      `<ol><li>打开设置，新增自定义模型提供商。</li><li>接口类型选择 OpenAI API 或 OpenAI Compatible。</li><li>Base URL 填写 <code>${state.baseURL}/v1</code>，再粘贴皮皮虾 API Key。</li><li>模型名请从控制台模型列表复制，保存后开始对话。</li></ol>${snippet("Base URL", `${state.baseURL}/v1`)}`,
  },
  cline: {
    title: "Cline / Roo Code",
    body: () =>
      `<ol><li>在 VS Code 扩展设置中选择 OpenAI Compatible 提供商。</li><li>Base URL 填写 <code>${state.baseURL}/v1</code>。</li><li>输入皮皮虾 API Key，并填写控制台中可见的模型名称。</li><li>首次调用失败时先确认 API Key、模型权限和余额。</li></ol>${snippet("Base URL", `${state.baseURL}/v1`)}`,
  },
  node: {
  ...
  "http-sdk": {
    title: "Java / HTTP",
    body: () =>
      `<p>无论使用 Java、Go、PHP 还是其他语言，核心都是向 OpenAI 兼容 HTTP 接口发送请求。</p>${snippet("Java HttpClient", `HttpRequest request = HttpRequest.newBuilder()\n    .uri(URI.create("${state.baseURL}/v1/chat/completions"))\n    .header("Authorization", "Bearer " + apiKey)\n    .header("User-Agent", "pipixia-client/1.0")\n    .header("Content-Type", "application/json")\n    .POST(HttpRequest.BodyPublishers.ofString(body))\n    .build();`)}`,
  },
  ccswitch: {
    title: "CCSwitch",
    body: () => `<p>CCSwitch 用于在本机统一管理 Claude Code、Codex 等客户端的供应商配置。请先升级到支持自定义 Provider 的版本，再添加皮皮虾 AI 供应商。</p>
      <p>皮皮虾供应商启用后的实际截图见上方步骤 3。需要手动新增时，使用 CCSwitch 右上角「+」进入配置。</p>
      <h3>Claude Code</h3>
      ...
      <p><b>生图 Key 说明：</b>生图 Key 与 CCSwitch 的编程 Key 都属于同一账户，可使用账户余额或各自绑定的套餐权益；实际扣费来源还取决于各 Key 选择的余额或套餐及套餐覆盖范围。切换本页顶部端点时，只需同步修改 <code>config.json</code> 的 <code>base_url</code>。</p><p><b>说明：</b>CCSwitch 中切换 API 端点时，只修改 Endpoint/Base URL。遇到 429 时，请先检查余额、模型配额以及套餐/分组权益，不要只当作限流。</p>`,
  },
  "chat-api": {
    title: "聊天补全 API",
    body: () =>
      `${snippet("cURL", `curl "${state.baseURL}/v1/chat/completions" \\\n  -H "User-Agent: pipixia-client/1.0" \\\n  -H "Authorization: Bearer $PIPIXIA_API_KEY" \\\n  -H "Content-Type: application/json" \\\n  -d '{"model":"gpt-5.4-mini","messages":[{"role":"user","content":"你好"}]}'`)}`,
  },
  "stream-api": {
    title: "流式输出",
    body: () =>
      `<p>将 <code>stream</code> 设为 <code>true</code>。客户端需要按 Server-Sent Events 流持续读取响应。</p>${snippet("cURL", `curl -N "${state.baseURL}/v1/chat/completions" \\\n  -H "User-Agent: pipixia-client/1.0" \\\n  -H "Authorization: Bearer $PIPIXIA_API_KEY" \\\n  -H "Content-Type: application/json" \\\n  -d '{"model":"gpt-5.4-mini","stream":true,"messages":[{"role":"user","content":"写一句问候"}]}'`)}`,
  },
  "responses-api": {
    title: "Responses API",
    body: () =>
      `${snippet("cURL", `curl "${state.baseURL}/v1/responses" \\\n  -H "User-Agent: pipixia-client/1.0" \\\n  -H "Authorization: Bearer $PIPIXIA_API_KEY" \\\n  -H "Content-Type: application/json" \\\n  -d '{"model":"gpt-5.4-mini","input":"你好"}'`)}`,
  },
  "image-api": {
    title: "图片生成 API",
    body: () =>
      `<p><b>推荐给 Codex 用户的生图方式：</b>CCSwitch 继续使用<b>普通编程分组 Key</b>运行 Codex；另外新建一把绑定<b>生图 API 分组</b>的 Key，只让项目里的生图 Skill 读取和调用。</p><ol><li>在控制台“API 密钥”新建一把 Key，分组选择<b>生图 API 分组</b>。不要把正在给 CCSwitch 使用的编程 Key 改到生图分组。</li><li>在需要生图的项目根目录创建 <code>.pipixia-image/config.json</code>，填入生图 Key、地址和图片模型。</li><li>将 <code>.pipixia-image/config.json</code> 加入该项目 <code>.gitignore</code>，确保绝不提交到 Git。</li><li>把下方“给 Codex 的提示词”发到该项目的 Codex 对话中。完成后，以后直接说“调用皮皮虾生图 Skill 生成一张……”即可。</li></ol>${snippet(".pipixia-image/config.json", `{\n  "base_url": "${state.baseURL}/v1",\n  "api_key": "在这里粘贴生图 API 分组 Key",\n  "model": "在控制台模型列表确认的图片模型名称"\n}`)}${snippet(".gitignore", `.pipixia-image/config.json`)}${snippet("给 Codex 的提示词", `请在当前项目创建一个本地生图 Skill。只读取项目根目录 .pipixia-image/config.json 中的 base_url、api_key、model，通过 POST {base_url}/images/generations 调用皮皮虾 AI 生图，请求头必须包含 User-Agent: pipixia-client/1.0。\n\n创建 .codex/skills/pipixia-image/SKILL.md 和可重复调用的本地脚本：支持 data[].url 与 data[].b64_json 响应，将图片保存到 output/images/。确保 .pipixia-image/config.json 已加入 .gitignore。严禁把 API Key 写进源码、日志、命令输出、Git 或生成文件；成功后只汇报图片文件路径、模型和必要错误信息。先检查当前项目技术栈，沿用已有脚本约定。`)}<p><b>只有在您自己写脚本或接入第三方生图客户端时</b>，才直接使用下面的 HTTP 示例。该示例中的 <code>$PIPIXIA_IMAGE_API_KEY</code> 必须是生图 API 分组的 Key。</p>${snippet("cURL", `curl "${state.baseURL}/v1/images/generations" \\\n  -H "User-Agent: pipixia-client/1.0" \\\n  -H "Authorization: Bearer $PIPIXIA_IMAGE_API_KEY" \\\n  -H "Content-Type: application/json" \\\n  -d '{"model":"图片模型名称","prompt":"一幅夜晚海边的水彩画","size":"1024x1024"}'`)}`,
  },
  "models-api": {
    title: "获取模型列表",
    body: () =>
      `<p>返回结果会受您的 API Key、分组和权限影响，因此它是确认可用模型名称的直接依据。</p>${snippet("cURL", `curl "${state.baseURL}/v1/models" \\\n  -H "User-Agent: pipixia-client/1.0" \\\n  -H "Authorization: Bearer $PIPIXIA_API_KEY"`)}`,
  },
};

function snippet(label, value) {
  return `<div class="code-panel"><div class="code-panel-head"><span>${label}</span><button class="copy-button" type="button" data-dialog-copy="${encodeURIComponent(value)}">复制</button></div><pre tabindex="0" aria-label="${escapeHTML(label)} 代码示例"><code>${escapeHTML(value)}</code></pre></div>`;
}
